<template>
  <div class="main-container">
      ...
      <template #default>
        ...
        <n-space style="float: right; padding-bottom: 20px">
          ...
          <n-button
            ...
            >重置</n-button
          >
          <n-button type="primary" @click="new_address_func" size="small"
            >新建定时任务</n-button
          >
          <n-button type="info" @click="task_monitor" size="small"
            >任务监控</n-button
          >
          <n-button type="primary" @click="export_excel" size="small"
            >导出EXCEL</n-button
          >
        </n-space>
        <n-data-table
    ...
    <ModalDialog
      ref="EditmodalDialog"
      title="编辑celery任务"
      @confirm="EditFormConfirm"
      :style="{ height: '600px', width: '500px' }"
    >
      <template #content>
        <DataForm
          ref="EdititemDataFormRef"
          :options="EdititemFormOptions"
          :form-config="{
            labelWidth: 80
          }"
        />
...
export default defineComponent({
  ...
  setup() {
      ...
      },
      {
        ...
        render: (formItem) => {
          return h(NSelect, {
            ...
            'on-update:value': crontab_func.bind(formItem.value.value)
          })
        }
      },
      {
        ...
        render: (formItem) => {
          return h(NSelect, {
            ...
            'on-update:value': interval_func.bind(formItem.value.value)
          })
        }
      },
      {
      ...
      useTableColumn(
        [
          ...
          },
          {
            ...
            render: (rowData) => {
              if (!rowData.enabled) {
                return h(NButton, { type: 'error', size: 'tiny' }, () =>
                  h('span', {}, '未启用')
                )
              } else {
                return h(NButton, { type: 'primary', size: 'tiny' }, () =>
                  h('span', {}, '已启用')
                )
              }
            }
            ...
            render: (rowData) => {
              if (rowData.enabled) {
                return h(
                  ...
                  {
                    trigger: () =>
                      h(NButton, { type: 'warning', size: 'tiny' }, () =>
                        h('span', {}, '停止')
                      ),
                ...
                return h(
                  ...
                  {
                    trigger: () =>
                      h(NButton, { type: 'success', size: 'tiny' }, () =>
                        h('span', {}, '开启')
                      ),
    ...
    function doRefresh() {
      ...
      if (searchDataFormRef.value === null) {
        ...
        request_url =
          getperiodic_taskList +
          '?name=' +
          searchDataFormRef.value.name +
          '&task=' +
          searchDataFormRef.value.task
      }
      get({
        url: request_url,
        data: () => {
    ...
    function CreateSubmitConfirm() {
      ...
      formdata.append(
        'crontab',
        create_info['crontab'] ? create_info['crontab'] : ''
      )
      formdata.append(
        'interval',
        create_info['interval'] ? create_info['interval'] : ''
      )
    ...
    function task_monitor() {
      ...
      window.open(`http://${serverIp}:5555/`)
    }
    ...
    function get_queue() {
      get({
        ...
        let queue_list: { value: any; label: any }[] = []
        ...
        nextTick(() => {
          EdititemFormOptions[4].optionItems.splice(0, 0, {
            value: '',
            label: '-----'
          })
          itemFormOptions[4].optionItems.splice(0, 0, {
            value: '',
            label: '-----'
          })
        })
    ...
    function edit_task(item) {
      ...
      EdititemFormOptions.forEach((it) => {
        ...
        if (key === 'u_location') {
          it.value.value =
            item['u_location_start'] + '-' + item['u_location_end']
        } else {
    ...
    function EditFormConfirm() {
      ...
      formdata.append(
        'crontab',
        edit_info['crontab'] ? edit_info['crontab'] : ''
      )
      formdata.append(
        'interval',
        edit_info['interval'] ? edit_info['interval'] : ''
      )
    ...
    function get_crontab_schedules() {
      get({
        ...
        nextTick(() => {
          EdititemFormOptions[7].optionItems.splice(0, 0, {
            value: '',
            label: '-----'
          })
          itemFormOptions[7].optionItems.splice(0, 0, {
            value: '',
            label: '-----'
          })
        })
    ...
    function get_interval_schedule() {
      get({
        ...
        if (res.results.length) {
          ...
          nextTick(() => {
            EdititemFormOptions[8].optionItems.splice(0, 0, {
              value: '',
              label: '-----'
            })
            itemFormOptions[8].optionItems.splice(0, 0, {
              value: '',
              label: '-----'
            })
          })
