  ...
  <div class="main-container">
    <n-space>
      ...
      <n-button
        ...
        size="small"
        @click="unBrush"
        v-show="showCancelBrushBtn"
        >取消框选</n-button
      >
      <n-button
        type="info"
        size="small"
        @click="nodeAlignmentbyY"
        v-show="showNodeAlignmentBtn"
        >横向对齐</n-button
      >
      <n-button
        type="info"
        size="small"
        @click="nodeAlignmentbyX"
        v-show="showNodeAlignmentBtn"
        >竖向对齐</n-button
      >
    </n-space>
        ...
        >
          ...
          >
            <tr>
              <td>设备名</td>
              <td>
                <p>{{ popover_node.name }}</p>
              </td>
            </tr>
            <tr>
              <td>设备IP</td>
              <td>
                <p>{{ popover_node.manage_ip }}</p>
              </td>
            </tr>
            <tr>
              <td>位置</td>
              <td>
                <p>{{ popover_node.location }}</p>
              </td>
            </tr>
            <tr>
              <td>序列号</td>
              <td>
                <p>{{ popover_node.serial_num }}</p>
              </td>
            </tr>
            <tr>
              <td>厂商型号</td>
              <td>
                <p>{{ popover_node.vendor_model }}</p>
              </td>
            </tr>
            <tr>
              <td>维保到期</td>
              <td>
                <p>{{ popover_node.expire }}</p>
              </td>
            </tr></n-table
          >
        </div>
        ...
        >
          ...
          >
            <tr>
              <td>左设备</td>
              <td>
                <p>{{ popover_link.left_device }}</p>
              </td>
            </tr>
            <tr>
              <td>左接口</td>
              <td>
                <p>{{ popover_link.left_int }}</p>
              </td>
            </tr>
            <tr>
              <td>左接口IP</td>
              <td>
                <p>{{ popover_link.left_int_ip }}</p>
              </td>
            </tr>
            <tr>
              <td>右设备</td>
              <td>
                <p>{{ popover_link.right_device }}</p>
              </td>
            </tr>
            <tr>
              <td>右接口</td>
              <td>
                <p>{{ popover_link.right_int }}</p>
              </td>
            </tr>
            <tr>
              <td>右接口IP</td>
              <td>
                <p>{{ popover_link.right_int_ip }}</p>
              </td>
            </tr>
            <tr>
              <td>速率</td>
              <td>
                <p>{{ popover_link.speed }}</p>
              </td>
            </tr>
          </n-table>
        </div>
      ...
      <n-drawer-content :title="dblclick_node_model.name" closable>
        <n-form ref="drawerFormRef" :model="dblclick_node_model">
          <n-form-item path="name" label="节点名">
            <n-input
              v-model:value="dblclick_node_model.name"
              :disabled="true"
            />
          </n-form-item>
          ...
          <n-row :gutter="[0, 24]">
            <n-col :span="24">
              <div style="display: flex; justify-content: flex-end">
                <n-button round type="primary" @click="drawerChangeNode">
                  保存
                </n-button>
              </div>
            </n-col>
      ...
      <n-drawer-content title="编辑链路" closable>
        <n-form ref="drawerLinkFormRef" :model="dblclick_link_model">
          <n-form-item path="source" label="源节点">
            <n-input
              v-model:value="dblclick_link_model.source.name"
              :disabled="true"
            />
          </n-form-item>
          <n-form-item path="source_manage_ip" label="源节点管理IP">
            <n-input
              v-model:value="dblclick_link_model.source_manage_ip"
              :disabled="true"
            />
          </n-form-item>
          <n-form-item path="source_interfaces" label="源节点接口">
            <n-input
              v-model:value="dblclick_link_model.source_interfaces"
              :disabled="true"
            />
          </n-form-item>
          <n-form-item path="source_interface_ip" label="源节点接口IP">
            <n-input
              v-model:value="dblclick_link_model.source_interface_ip"
              :disabled="true"
            />
          </n-form-item>
          <n-form-item path="target" label="目标节点">
            <n-input
              v-model:value="dblclick_link_model.target.name"
              :disabled="true"
            />
          </n-form-item>
          <n-form-item path="target_manage_ip" label="目标节点管理IP">
            <n-input
              v-model:value="dblclick_link_model.target_manage_ip"
              :disabled="true"
            />
          </n-form-item>
          <n-form-item path="target_interfaces" label="目标节点接口">
            <n-input
              v-model:value="dblclick_link_model.target_interfaces"
              :disabled="true"
            />
          </n-form-item>
          <n-form-item path="target_interface_ip" label="目标节点接口IP">
            <n-input
              v-model:value="dblclick_link_model.target_interface_ip"
              :disabled="true"
            />
          </n-form-item>
          <n-form-item path="speed" label="速率">
            <n-input-number
              v-model:value="dblclick_link_model.speed"
              :disabled="true"
            />
          </n-form-item>
          <n-form-item path="color" label="线路颜色">
            <n-color-picker
              v-model:value="dblclick_link_model.color"
              :show-alpha="false"
            />
          </n-form-item>
          <n-row :gutter="[0, 24]">
            <n-col :span="24">
              <div style="display: flex; justify-content: flex-end">
                <n-button round type="primary" @click="drawerChangeLink">
                  保存
                </n-button>
              </div>
            </n-col>
    ...
    <n-modal v-model:show="showAddGraphModal">
      ...
      >
        ...
        <template #footer>
          <div style="display: flex; justify-content: flex-end">
            <n-button round type="primary" @click="onAddGraphConfirm">
              新建
            </n-button>
          </div>
        </template>
    ...
    <n-modal v-model:show="showAddNodeModal">
      ...
      >
        ...
        <n-form ref="addNodeFormRef1" :model="add_node_query_model" inline>
          ...
          <n-form-item path="idc" label="idc">
            <n-select
              :options="idcOptions"
              filterable
              v-model:value="add_node_query_model.idc"
            />
          </n-form-item>
        </n-form>
        ...
        <template #footer>
          <div style="display: flex; justify-content: flex-end">
            <n-button round type="primary" @click="onAddNodeConfirm">
              添加
            </n-button>
          </div>
        </template>
    ...
    <!--连线管理-->
    <n-modal v-model:show="showManageLinkModal">
      <n-card
        style="width: 800px"
        title="连线管理"
        :bordered="false"
      ...
      >
        ...
        <TableBody>
          <template #default>
            <n-data-table
              :data="graph.links"
              :columns="manageLinksTableColumns"
              :row-key="rowKey"
              @update:checked-row-keys="handleSelectionChange"
            />
          </template>
    ...
    <!--新增手动连线-->
    <n-modal v-model:show="showAddManualLinkModal">
      <n-card
        style="width: 800px"
        title="新增手动连线"
        :bordered="false"
      ...
      >
        <template #default>
          ...
          >
            <n-form-item label="A端点">
              <n-select
                v-model:value="manaul_model.manaul_source"
                :options="manaul_model.option"
                @update:value="manaul_link_get_a_interface"
                filterable
              />
            </n-form-item>
            <n-form-item label="A接口">
              <n-select
                v-model:value="manaul_model.manaul_source_interface"
                :options="manaul_model.source_interface_option"
                filterable
              />
            </n-form-item>
            <n-form-item label="B端点">
              <n-select
                v-model:value="manaul_model.manaul_target"
                :options="manaul_model.option"
                @update:value="manaul_link_get_b_interface"
                filterable
              />
            </n-form-item>
            <n-form-item label="B接口">
              <n-select
                v-model:value="manaul_model.manaul_target_interface"
                :options="manaul_model.target_interface_option"
                filterable
              />
            </n-form-item>
          </n-form>
        ...
        <template #footer>
          <div style="display: flex; justify-content: flex-end">
            <n-button round type="primary" @click="add_manaul_btn">
              添加
            </n-button>
          </div>
        </template>
...
const manaul_model = ref({
  option: [],
  source_interface_option: [],
  target_interface_option: [],
  manaul_source: '',
  manaul_source_interface: '',
  manaul_target: '',
  manaul_target_interface: ''
})
...
const height =
  Math.max(document.documentElement.clientHeight, window.innerHeight || 0) - 250
const color = d3
  .scaleOrdinal()
...
function select_topology() {
  ...
  get({
    ...
    if (res.code == 200) {
      ...
      manaul_model.value.option = res.data.nodes.map((v) => ({
        label: v.name + '(' + v.manage_ip + ')',
        value: v.manage_ip
      }))
      if (group.value !== null) {
...
function init_links() {
  ...
  svg_link.on('click', function (event, d) {
    if (d.source.x < d.target.x) {
      popover_link.value.left_device =
        d.source.id + '(' + d.source.manage_ip + ')'
      popover_link.value.right_device =
        d.target.id + '(' + d.target.manage_ip + ')'
...
function init_svg() {
  ...
  simulation = d3
    ...
    .force(
      'charge',
      d3.forceManyBody().strength(0.001).distanceMax(0.001).distanceMin(0.001)
    )
  ...
  graph.value.nodes.forEach((node) => {
    brushNode.value.forEach((_node) => {
      if (node.device_id == _node.device_id) {
        node.fx =
          node.fx == AlignmentBaseX.value ? node.fx : AlignmentBaseX.value
      }
    })
  })
  nextTick(() => {
...
const nodeAlignmentbyY = () => {
  ...
  graph.value.nodes.forEach((node) => {
    brushNode.value.forEach((_node) => {
      if (node.device_id == _node.device_id) {
        node.fy =
          node.fy == AlignmentBaseY.value ? node.fy : AlignmentBaseY.value
      }
    })
  })
  nextTick(() => {
...
function uniqueDevice(arr) {
  let res = new Map()
  return arr.filter(
    (arr) => !res.has(arr.device_id) && res.set(arr.device_id, 1)
  )
}
// 设备框选函数
function Brush() {
      ...
      .on('brush', function brushing(event) {
        ...
        if (brushNode.value.length != 0) {
          brushNode.value.forEach((node) => {
            ...
            node.fx =
              node.x - (last_coordinate_x.value - current_coordinate_x.value)
            node.fy =
              node.y - (last_coordinate_y.value - current_coordinate_y.value)
...
function manageLinks() {
  showManageLinkModal.value = true
}
// 新增手动连线
function add_link() {
  showAddManualLinkModal.value = true
}
// 手动连线下拉框值发生改变时，获取对应设备的接口列表
function manaul_link_get_a_interface() {
  get({
    url: topology_show,
    data: () => {
      return {
        get_interface_by_manage_ip: manaul_model.value.manaul_source
      }
    }
  }).then((res) => {
    nextTick(() => {
      manaul_model.value.source_interface_option = res.data
        .filter((v) => {
          // 在这里添加你的过滤条件
          // 返回 true 表示满足条件，将被保留
          // 返回 false 表示不满足条件，将被过滤掉
          return v.status === 'up'
        })
        .map((v) => ({
          label: v.interface,
...
function manaul_link_get_b_interface() {
  get({
    url: topology_show,
    data: () => {
      return {
        get_interface_by_manage_ip: manaul_model.value.manaul_target
      }
    }
  }).then((res) => {
    nextTick(() => {
      manaul_model.value.target_interface_option = res.data
        .filter((v) => {
          // 在这里添加你的过滤条件
          // 返回 true 表示满足条件，将被保留
          // 返回 false 表示不满足条件，将被过滤掉
...
// 新增手动连线模态框的添加按钮  post提交数据
function add_manaul_btn() {
  console.log(manaul_model.value)
  let a_name = graph.value.nodes.filter((v) => {
    return v.manage_ip == manaul_model.value.manaul_source
  })
  let b_name = graph.value.nodes.filter((v) => {
    return v.manage_ip == manaul_model.value.manaul_target
  })
  post({
    url: topology_show,
    data: {
      name: graph.value.name,
      a_name: a_name[0].name,
      a_device: manaul_model.value.manaul_source,
      b_name: b_name[0].name,
      b_device: manaul_model.value.manaul_target,
      a_interface: manaul_model.value.manaul_source_interface,
      b_interface: manaul_model.value.manaul_target_interface
    }
  }).then((res) => {
    if (res.code == 200) {
      message.success(res.msg)
...
// 连线管理模态框
const showManageLinkModal = ref(false)
// 手动连线模态框
const showAddManualLinkModal = ref(false)
...
function onAddNodeConfirm() {
  ...
  post({
    url: topology_show,
    data: { name: topology_value.value, add_nodes: add_nodes }
  }).then((res) => {
    if (res.code == 200) {
      message.success(res.msg)
