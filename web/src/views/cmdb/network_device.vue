<template>
    <div class="main-container">
        <TableBody>
                ...
                <TableHeader :show-filter="false" title="查询条件" @search="onSearch" @reset-search="onResetSearch">
                    ...
                    <template #top-right>
                        ...
                        <n-button type="warning" size="small" @click="btnClick">
                            数据导入</n-button>
                        <n-button type="primary" size="small" @click="export_excel">
                            数据导出</n-button>
                        <n-button type="primary" size="small" @click="connect_collect">
                            关联采集</n-button>
            ...
            <template #default>
                <DataForm ref="searchForm" :form-config="{ labelWidth: 60 }" :options="conditionItems" preset="grid-item" />
                ...
                <n-data-table :loading="tableLoading" :data="dataList" :columns="tableColumns" :single-line="!bordered"
                    :row-key="rowKey" @update:checked-row-keys="handleCheck" @update:expanded-row-keys="handleExpand"
                    :scroll-x="1800" />
            </template>
            <template #footer>
        ...
        <ModalDialog ref="modalDialog" title="资产数据维护" :segmented="segmented" display-directive="show"
            @confirm="EditConfirm">
            <template #content>
                <DataForm ref="EditDataFormRef" :form-config="{ labelWidth: 60 }" preset="grid-item"
        ...
        <ModalDialog ref="device_import_modalDialog" title="资产数据录入" @confirm="importDataFormConfirm"
            :style="{ height: '500px' }">
            <template #content>
                <DataForm ref="importDataFormRef" :form-config="{
                    labelWidth: 60,
                }" preset="grid-item" :options="importFormOptions" />
            </template>
        </ModalDialog>

        <ModalDialog ref="ChangeLogmodalDialog" title="变更轨迹" :style="{ height: '768px', width: '1366px' }">
            <template #content>
                <n-data-table :data="change_log_list.slice(
                    (change_log_page - 1) * change_log_pageSize,
                    change_log_page * change_log_pageSize
                )
                    " size="small" :columns="change_log_tableColumns" :row-key="rowKey" :loading="tableLoading" />
                <div class="flex justify-center">
                    <n-pagination v-model:page="change_log_page" :page-count="change_log_pageCount" show-size-picker
                        :page-sizes="change_log_pageSizes" />
                </div>
            </template>
        </ModalDialog>

        <ModalDialog ref="show_password_modalDialog" title="查看管理账户信息?" :style="{ height: '500px', width: '500px' }"
            @confirm="second_onConfirm">
            <template #content>
                <n-space>
                    <h3>请输入二级密码</h3>
                    <n-input type="password" show-password-on="mousedown" placeholder="回车查看密码"
                        v-model:value="second_password" @keyup.enter.native="second_onConfirm" />
                </n-space>
            </template>
        </ModalDialog>
        <ModalDialog ref="bind_ip_modalDialog" title="关联IP" :style="{ height: '400px', width: '300px' }"
            @confirm="BindIpConfirm">
            <template #content>
                <n-form label-placement="left" :size="size" label-width="auto" require-mark-placement="right-hanging"
                    :style="{
                        maxWidth: '640px',
                    }" :model="bind_ip_form">
        ...
        <ModalDialog ref="connect_account_modalDialog" title="关联设备管理账户" :style="{ height: '500px', width: '500px' }"
            @confirm="ConnectAccountConfirm">
            <template #content>
                <DataForm ref="connect_account_DataFormRef" :form-config="{
                    labelWidth: 60,
                }" :options="connect_account_FormOptions" />
            </template>
        </ModalDialog>
        <ModalDialog ref="account_modalDialog" title="账户信息" :style="{ height: '500px', width: '800px' }">
            <template #content>
                <n-data-table :data="account_list.slice(
                    (account_page - 1) * account_pageSize,
                    account_page * account_pageSize
                )
                    " :columns="account_tableColumns" :row-key="rowKey" :loading="tableLoading" />
            </template>
        </ModalDialog>

        <n-modal v-model:show="collect_modalDialog" class="modal-dialog-wrapper" header-style="padding: 10px 20px"
            :mask-closable="false" :style="bodyStyle">
            <n-card style="width: 600px" title="关联采集方案" :bordered="false" size="huge" role="dialog" aria-modal="true">
                <n-form ref="formRef" :model="formRef" label-placement="left" label-align="left" :label-width="160" :style="{
                    maxWidth: '640px',
                }">
                    <n-form-item label="当前选中设备条目:">{{
                        checkedRowKeysRef.length
                    }}</n-form-item>

                    <n-form-item label="采集方案">
                        <n-select v-model:value="selectCollectValues" filterable placeholder="勾选采集方案"
        ...
        <n-modal v-model:show="import_show" preset="dialog" header-style="padding: 10px 20px" title="设备批量表格导入"
            :style="{ height: '230px', width: '600px' }" :mask-closable="false">
    ...
    setup() {
                                            ...
                                            [
                                                h(
                                                    NButton,
                                                    {
                                                        text: true,
                                                        color: "#204d74",
                                                        onclick: show_account_handleClick.bind(
                                                            null,
                                                            rowData
                                                        ),
                                                    },
        ...
        function NewConnectCollect() {
            ...
            checkedRowKeysRef.value.forEach((devid) => {
                ...
                let dev_detail_info = table.dataList.filter(
                    (item) => item.id === devid
                )[0];
        ...
        function doRefresh() {
            ...
            if (info_host_list) {
                request_url =
                    getNetworkDeviceList + "?search_host_list=" + info_host_list;
            }
            if (info_host_list === "") {
                // message.error("检索无数据")
                request_url = getNetworkDeviceList;
            ...
            if (importDataFormRef.value?.validator()) {
                ...
                post({
                    ...
                    if (res.code === 400) {
                        ...
                        nextTick(() => {
                            message.success(
                                "设备录入成功,SN号:" + JSON.stringify(import_form.serial_num)
                            );
                            doRefresh();
                        });
                    }
        ...
        function export_excel() {
            ...
            if (checkedRowKeysRef.value.length > 0) {
                ...
                //console.log(table.dataList)
                let check_list = [{}];
                table.dataList.forEach((ele: { id: number }) => {
                    if (checkedRowKeysRef.value.indexOf(ele.id) !== -1) {
                        check_list.push(ele);
                    }
                });
