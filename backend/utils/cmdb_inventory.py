...
def _get_inventory_element(
    ...
    return typ(
        ...
        port=data['data']['ssh']['port'] if 'ssh' in data['data'].keys() else data['data']['telnet']['port'],
        username=data['data']['ssh']['username'] if 'ssh' in data['data'].keys() else data['data']['telnet']['username'],
        password=data['data']['ssh']['password'] if 'ssh' in data['data'].keys() else data['data']['telnet']['password'],
        platform=data.get("platform"),
...
class CMDBInventory(InventoryPlugin):
    ...
    def load(self) -> Inventory:
        ...
        for name, host_dict in hosts_dict.items():
            if 'ssh' in host_dict['data'].keys() or 'telnet' in host_dict['data'].keys():
