...
class BatManFsm(object):
    ...
    def get_structured_data(raw_output, platform, command):
        ...
        template_dir = BASE_DIR + '/utils/connect_layer/zetmiko/templates'
...
class BatManMain(object):
    ...
    # 山石防火墙专用配置引擎
    @staticmethod
    def hillstone_config_cmds(*cmds, **dev_info):
        paths = []
        filename = 'automation/' + dev_info['ip'] + '/' + 'config_' + str(int(time.time())) + '.txt'
        try:
            with ConnectHandler(**dev_info) as dev_connection:
    ...
    def hillstone_config_v2(*cmds, **dev_info):
        ...
        try:
            with ConnectHandler(**dev_info) as dev_connection:
                ...
                # dev_connection.read_until_prompt_or_pattern(pattern=r"\[y\]/n:")
                # dev_connection.write_channel('y')
                # dev_connection.read_until_prompt_or_pattern(pattern=r"y/\[n\]:")
                # dev_connection.write_channel('y')
                # dev_connection.read_until_prompt_or_pattern(pattern=r"#")
                # 自定义保存结束
                # content += dev_connection.save_config(cmd="save", confirm=True, confirm_response="y")
                dev_connection.close_session_log()
                dev_connection.disconnect()
        #
        # except Exception as e:
    ...
    # 山石防火墙配置过程， 不保存配置
    @staticmethod
    def hillstone_config_cmds_no_save(*cmds, **dev_info):
        paths = []
        try:
            with ConnectHandler(**dev_info) as dev_connection:
                # prompt = dev_connection.find_prompt()  # 找出设备的prompt
                ...
                content = dev_connection.send_config_set(config_commands=cmds,
                                                         ...
                                                         enter_config_mode=True, )
                # content += dev_connection.exit_config_mode(exit_config="exit", pattern="#")
                # content += dev_connection.save_config('save', 'y')
                # content += dev_connection.save_config(cmd="save", confirm=True, confirm_response="y")
                dev_connection.close_session_log()
                dev_connection.disconnect()
                paths.append(filename)
        ...
        #     print(traceback.print_exc())
            ...
            print('[Error 1] Authentication failed.{}'.format(str(e)))
            BatManMongo.insert_failed_logs(hostip=dev_info['ip'], device_type=dev_info['device_type'], info=error_text)
            return False, '[Error 1] Authentication failed.{}'.format(str(e))
        except NetmikoTimeoutException as e:  # 登录超时报错记录
            error_text = '[Error 2] Connection timed out.{}'.format(str(e))
            print('[Error 2] Connection timed out.{}'.format(str(e)))
            BatManMongo.insert_failed_logs(hostip=dev_info['ip'], device_type=dev_info['device_type'], info=error_text)
            return False, '[Error 2] Connection timed out.{}'.format(str(e))
        except Exception as e:  # 未知报错记录
            error_text = '[Error 3] Unknown error. {}'.format(str(e))
            print('[Error 3] Unknown error. {}'.format(str(e)))
            # 采集失败的记录日志
            BatManMongo.insert_failed_logs(hostip=dev_info['ip'], device_type=dev_info['device_type'], info=error_text)
            # print(str(e))
            return False, '[Error 3] Unknown error. {}'.format(str(e))
        return paths, ''
...
if __name__ == '__main__':
    ...
    res = BatManMain.info_fsm(path='route_file/display_ip_routing-table.txt', fsm_platform='hp_comware')
