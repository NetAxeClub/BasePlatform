...
from ..automation.sec_main import SecPolicyMain, address_set, FirewallMain, service_set, dnat_mongo, config_dnat
...
class FirewallBase(APIView):
    ...
    def get(self, request):
        ...
        if 'get_firewall_list' in get_param.keys():
            _res = get_firewall_list()
            res = json.dumps({'results': list(_res), 'count': len(_res),
                              'code': 200})
            cache.set('get_firewall_list' + str(request.user.username), res, 60)
            return JsonResponse({'results': list(_res), 'count': len(_res), 'code': 200})
        # 查询公网IP和设备的对应关系
        if "public_ip" in get_param.keys():
            mongo_res = MongoNetOps.dnat_ip_owner_query(ip=get_param['public_ip'])
            if mongo_res:
                _res = get_firewall_list()
                res = json.dumps({'results': list(_res), 'count': len(_res),
                                  'code': 200})
                cache.set('sec_public_ip' + str(request.user.username) + get_param['public_ip'], res, 60)
                return JsonResponse({'results': list(_res), 'count': len(_res), 'code': 200})
            else:
                return JsonResponse({'results': [], 'count': 0, 'code': 400})

        return JsonResponse({'code': 200})


# 地址组V2
class AddressSet(View):
    ...
    def post(self, request):
        ...
        if request.user.is_superuser:
            ...
            post_param = request.body.decode('utf-8')
            post_param = json.loads(post_param)
            # print("地址组", json.loads(post_param))
            # 更新单个设备地址组信息(山石)
            if all(k in post_param for k in ("vendor", "update_device", "hostip")):
                ...
                return HttpResponse(json.dumps(dict(code=400, message='操作不被允许')),
                                    content_type="application/json")
            ...
            return HttpResponse(json.dumps(dict(code=400, message='没有获取到用户信息')),
                                content_type="application/json")
            ...
            if user.has_perm('automation.change_autoflow'):
                ...
                if all(k in post_param for k in ("vendor", "hostip", "hostid")):
                    ...
                    if schema_res:
                        ...
                        res = service_set.apply_async(kwargs=post_param, queue=CELERY_QUEUE,
                                                      retry=True)  # config_backup
                        if str(res) == 'None':
                            print('forget')
                            res.forget()
                ...
                return JsonResponse(dict(code=400, message='用户没有权限'))
        else:
            return JsonResponse(dict(code=400, message='没有获取到用户信息'))
...
class DestAddTranslate(APIView):
    ...
    def post(self, request):
        ...
        if post_param.get('user'):
            user = AuthUsers.objects.filter(username=post_param['user'])
            if user:
                post_param['origin'] = '运营平台'
                post_param['user'] = str(post_param['user'])
            else:
                return JsonResponse(dict(code=400, message='运营平台没有获取到用户信息'))
        elif request.user:
            # user = UserProfile.objects.get(username=request.user)
            post_param['origin'] = '运维平台'
            post_param['user'] = str(request.user.username)
        else:
            return JsonResponse(dict(code=400, message='没有匹配到用户信息'))
        # 更新单个设备DNAT信息
        if all(k in post_param for k in ("vendor", "update_device", "hostip")):
            if post_param['vendor'] == 'Hillstone':
