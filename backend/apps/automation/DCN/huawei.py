...
from apps.automation.utils.auto_main import BatManMain, HuaweiS
...
class HuaweiProc(BaseConn):
    ...
    def _version_proc(self, res):
        if isinstance(res, list):
            if res[0]['model'] and self.model__name is None:
                _vendor = AssetVendor.objects.get(name='华为')
                _model_instance, _action = AssetModel.objects.get_or_create(
        ...
        if res:
            if isinstance(res, dict):
                ...
                if self.model__name != res['productName']:
                    updata_data['model'], flag = AssetModel.objects.get_or_create(
                        vendor=AssetVendor.objects.get(name='华为'), name=res['productName']
                    )
            ...
            if isinstance(res, list):
                for _sysinfo in res:
                    ...
                    if self.model__name != _sysinfo['productName']:
                        updata_data['model'], flag = AssetModel.objects.get_or_create(
                            vendor=AssetVendor.objects.get(name='华为'), name=_sysinfo['productName']
                        )
    ...
    def _netcocnf_system_info(self, dev_sysinfo):
        print('dev_sysinfo', dev_sysinfo)
        if dev_sysinfo:
            if isinstance(dev_sysinfo, dict):
                model_q = AssetModel.objects.filter(name=dev_sysinfo['model'])
                if model_q:
                    NetworkDevice.objects.filter(manage_ip=self.hostip, status=0) \
                        .update(model=AssetModel.objects.get(name=dev_sysinfo['model']),
                                soft_version=dev_sysinfo['version'],
                                patch_version=dev_sysinfo['patch-version'])
                else:
                    model_q = AssetModel.objects.create(name=dev_sysinfo['model'],
                                                        vendor=AssetVendor.objects.get(alias='Huawei'))
                    NetworkDevice.objects.filter(manage_ip=self.hostip, status=0) \
            ...
            if isinstance(dev_sysinfo, list):
                for _sysinfo in dev_sysinfo:
                    model_q = AssetModel.objects.filter(name=_sysinfo['model'])
                    if model_q:
                        NetworkDevice.objects.filter(manage_ip=self.hostip, status=0) \
                            .update(model=AssetModel.objects.get(name=_sysinfo['model']),
                                    soft_version=_sysinfo['version'],
                                    patch_version=_sysinfo['patch-version'])
                    else:
                        model_q = AssetModel.objects.create(name=_sysinfo['model'],
                                                            vendor=AssetVendor.objects.get(alias='Huawei'))
                        NetworkDevice.objects.filter(manage_ip=self.hostip, status=0) \
                            .update(model=model_q,
                                    soft_version=_sysinfo['version'],
