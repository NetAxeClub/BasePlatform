...
from apps.asset.models import NetworkDevice, AssetModel, AssetVendor
from apps.automation.utils.auto_main import BatManMain
from apps.service.NETCONF.h3c_netconf import H3CinfoCollection, H3CSecPath
...
class H3cProc(BaseConn):
        ...
        for i in datas:
            ...
            if i.get('ServGrpList'):
                ...
                if isinstance(i['ServGrpList']['ServGrpItem'], str):
                    i['ServGrpList']['ServGrpItem'] = [i['ServGrpList']['ServGrpItem']]
                if isinstance(i['ServGrpList']['ServGrpItem'], list):
                    service += [{'object': x}
                                for x in i['ServGrpList']['ServGrpItem']]
                    for _tmp in i['ServGrpList']['ServGrpItem']:
                        ...
                        for _tmp in _tmp_service_q:
                            if 'items' in _tmp.keys():
                                for _item in _tmp['items']:
                                    _item['Type'] = type_map[_item['Type']]
                                    if 'StartSrcPort' in _item.keys():
                                        _item['StartSrcPort'] = int(_item['StartSrcPort'])
                                        _item['EndSrcPort'] = int(_item['EndSrcPort'])
                                        _item['StartDestPort'] = int(_item['StartDestPort'])
                                        _item['EndDestPort'] = int(_item['EndDestPort'])
                                    service_split.append(_item)
            if i.get('ServObjList'):
                ...
                if isinstance(i['ServObjList']['ServObjItem'], str):
                    i['ServObjList']['ServObjItem'] = [i['ServObjList']['ServObjItem']]
                if isinstance(i['ServObjList']['ServObjItem'], list):
                    for x in i['ServObjList']['ServObjItem']:
                        _item = json.loads(x)
                        if _item['Type'] in ["0", "1"]:
                            _item['Type'] = type_map[_item['Type']]
                            if 'StartSrcPort' in _item.keys():
                                _item['StartSrcPort'] = int(_item['StartSrcPort'])
                                _item['EndSrcPort'] = int(_item['EndSrcPort'])
                                ...
                                _item['EndDestPort'] = int(_item['EndDestPort'])
                        service_split.append(_item)
                        service.append(dict(item=_item))
            ...
            if i.get('SrcAddrList'):
                if isinstance(i['SrcAddrList']['SrcAddrItem'], str):
                    i['SrcAddrList']['SrcAddrItem'] = [i['SrcAddrList']['SrcAddrItem']]
                        ...
                        if src_addr_res:
                            for _src_addr in src_addr_res:
                                if 'ObjList' not in _src_addr.keys():
                                    continue
                                for _sub_addr in _src_addr['ObjList']:
                                    # 子网
            ...
            if i.get('DestAddrList'):
                if isinstance(i['DestAddrList']['DestAddrItem'], str):
                    i['DestAddrList']['DestAddrItem'] = [i['DestAddrList']['DestAddrItem']]
                        ...
                        if dst_addr_res:
                            for _dst_addr in dst_addr_res:
                                if 'ObjList' not in _dst_addr.keys():
                                    continue
                                for _sub_addr in _dst_addr['ObjList']:
                                    # 子网
                                    if _sub_addr['Type'] == 'subnet':
                                        if all(k in _sub_addr for k in (
                                                "SubnetIPv4Address", "IPv4Mask")):
            ...
            tmp['service'] = service
            tmp['service_split'] = service_split
        ...
        if isinstance(res, list):
            for i in res:
                if i['interface'].startswith('BAGG'):
                    ...
                    continue
                elif i['interface'].startswith('Loop'):
                    continue
                elif i['interface'].startswith('InLoop'):
                    continue
                if i['speed'].find('-') != -1:
                    i['speed'] = 'IRF'
                elif i['speed'].find('(') != -1:
    ...
    def _version_proc(self, res):
        ...
        if isinstance(res, list):
            for i in res:
                model = AssetModel.objects.filter(name=i['board_type'], vendor__alias='H3C')
                if model:
                    NetworkDevice.objects.filter(
                        manage_ip=self.hostip,
                        slot=int(i['slot'])).update(soft_version=i['version'],
                                                    model=AssetModel.objects.get(name=i['board_type'],
                                                                                 vendor__alias='H3C'))
                else:
                    NetworkDevice.objects.filter(
                        manage_ip=self.hostip,
                        slot=int(i['slot'])).update(soft_version=i['version'],
                                                    model=AssetModel.objects.create(name=i['board_type'],
                                                                                    vendor__alias='H3C'))
        return
        ...
        if isinstance(res, dict):
            ...
            if res.get('Model', ''):
                ...
                if self.model__name != tmp_model:
                    model_q, flag = AssetModel.objects.get_or_create(
                        name=tmp_model, vendor=AssetVendor.objects.get(alias='H3C'))
            ...
            for _physical in res:
                if _physical['SoftwareRev'] is None:
                    continue
            ...
            for dev in res:
                ...
                if isinstance(dev['Board'], list):
                    if 'Master' in [x['Role'] for x in dev['Board']]:
                        ...
                        if self.category__name == '防火墙':  # 因M9006 113 slot 硬件信息和irf的slot信息对应不上
                            NetworkDevice.objects.filter(manage_ip=self.hostip,
                                                         chassis=int(_chassis_num[0])).update(
                                ha_status=ha_status)
                        else:
                    ...
                    if dev['Board']['Role'] == 'Master':
                        ha_status = 1 if len(res) == 2 else 0
                    elif dev['Board']['Role'] == 'Standby':
                        ha_status = 2 if len(res) == 2 else 0
                    NetworkDevice.objects.filter(manage_ip=self.hostip,
                                                 slot=int(
                                                     dev['Board']['Slot']),
        ...
        if res:
            ...
            try:
                for i in res:
                    ...
                    if i['GlobalInfo']['GlobalStartPortNumber'] is not None:
                        global_port_start = int(i['GlobalInfo']['GlobalStartPortNumber'])
                    else:
                        global_port_start = 0
                    if i['GlobalInfo']['GlobalEndPortNumber'] is not None:
    ...
    def _netconf_global_nat_policy(self, res):
        if res:
            for i in res:
                ...
                global_port = []
                if isinstance(i['SrvObjGrpList']['ServiceObjGroup'], str):
                ...
                elif isinstance(i['SrvObjGrpList']['ServiceObjGroup'], list):
                    for _ser_obj_group in i['SrvObjGrpList']['ServiceObjGroup']:
                        if _ser_obj_group in self.ser_map.keys():
                            ser_tmp = self.ser_map[_ser_obj_group]
                            for _ser_obj in ser_tmp:
                                # print(_ser_obj)
                                global_port.append(
                                    dict(
