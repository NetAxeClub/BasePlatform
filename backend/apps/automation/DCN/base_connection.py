...
from apps.service.NETCONF.h3c_netconf import H3CinfoCollection
from apps.service.NETCONF.huawei_netconf import HuaweiCollection, HuaweiUSG
...
class BaseConn:
    def __init__(self, **kwargs):
        ...
        if kwargs.get('en_pwd'):
            self.netmiko_params['secret'] = kwargs['en_pwd']
        ...
        self.netconf_params = {}
        self.l2vpn = kwargs['l2vpn']
        if 'netconf' in kwargs['protocol']:
            self.netconf_params = {
                'ip': self.hostip,
                'port': kwargs['netconf_port'],
                'username': kwargs['netconf_username'],
                'password': kwargs['netconf_password'],
                'timeout': 200,  # float，连接超时时间，默认为100
                'session_timeout': 100,  # float，每个请求的超时时间，默认为60
            }
        ...
        try:
            with ConnectHandler(**self.netmiko_params) as dev_connection:
                ...
                if self.vendor_alias in ['H3C', 'Huawei']:
                    ...
                    if self.vendor_alias == 'Hillstone' and self.hostname is not None:
                        if self.hostname.find('(M)') != -1:
                            NetworkDevice.objects.filter(manage_ip=self.hostip).update(ha_status=1)
                        elif self.hostname.find('(B)') != -1:
                            NetworkDevice.objects.filter(manage_ip=self.hostip).update(ha_status=2)
                # print(prompt)
                for cmd in cmds:
            ...
            error_text = '[Error 3] ConfigInvalidException.{}'.format(str(e))
...
if __name__ == '__main__':
    kwargs = {'id': 30, 'serial_num': '1504905160005642', 'manage_ip': '10.254.6.54', 'name': 'A2.WXYY.PO.FW.G02',
              'soft_version': '5.0', 'vendor__name': '山石网科', 'vendor__alias': 'Hillstone', 'category__name': '防火墙',
              'framework__name': '二层', 'model__name': 'SG-6000-G2120', 'plan_id': 21, 'l2vpn': True,
              'patch_version': '', 'status': 0, 'idc__name': '合肥A2', 'auto_enable': True, 'ha_status': 1, 'chassis': 0,
              'slot': 0, 'bind_ip__ipaddr': None, 'username': 'hillstone', 'password': 'ax5(Hn=FicY}K5Ah', 'port': 22,
              'protocol': ['telnet', 'ssh']}
    conn = BaseConn(**kwargs)
    conn._send_cmd(['dis version'])
