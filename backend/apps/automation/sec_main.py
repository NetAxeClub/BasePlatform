...
from apps.automation.DCN.hillstone import HillstoneProc
...
from apps.automation.tasks import AutomationMongo
...
from utils.connect_layer.NETCONF.h3c_netconf import H3CSecPath, H3CinfoCollection
...
def service_desk_ops(**kwargs):
    code_id = kwargs['code_id']
    status = kwargs['status']
    user = kwargs['user']
    _get_instance_id = get_instance_id(user, code_id)
    if _get_instance_id:
        node_info_submit(_get_instance_id, status)
    else:
        pass
        # send_msg_sec_manage("ServiceDesk工单:{}\n无法获取用户的apikey,请手动触发流程至下一步".format(code_id))
    return
...
class FirewallMain(object):
    ...
    def flow_engine(self, *args, **kwargs):
        ...
        if isinstance(flag, bool):
            kwargs.pop('flag')
        if kwargs.get('origin') == '运营平台':
            _user = AuthUsers.objects.get(username=kwargs['commit_user'])
        else:
            # 获取用户实例
            _user = UserProfile.objects.get(username=kwargs['commit_user'])
        # 判断用户是否在automaiton_run_action组中并且bgbu符合
        # _user_bgbu = [x['name'] for x in _user.bgbu.filter(~Q(name='网络组')).values()]  # ['安全团队', 'DC运维']
        _user_bgbu = [x['name'] for x in _user.bgbu.all().values()]  # ['安全团队', 'DC运维']
        # 获取设备实例
        _device = NetworkDevice.objects.get(id=kwargs['device_id'])
        # _device_bgbu = [x['name'] for x in _user.bgbu.filter(~Q(name='网络组')).values()]
        _device_bgbu = [x['name'] for x in _device.bgbu.all().values()]  # ['消费者BG输入法', '医疗BU', '网络组', '安全团队', 'DC运维']
        # 求 用户的bgbu和设备的bgbu交集
        compare_res = list(set(_user_bgbu).intersection(set(_device_bgbu)))
        if len(compare_res) > 0:
            # 验证通过  创建流程
            flow_record = AutoFlow.objects.create(**kwargs)
            # 如果判断不需要下发动作，则直接返回
            if flag is False:
                flow_record.approve(by=_user)
                flow_record.publish(by=_user)
                flow_record.task_result = 'Configuration already exists, netconf xml is None'
                flow_record.save()
                sec_callback(flow_record.id)
                return
            if flow_record.origin == '运营平台':
                print("运营平台用户不具备一键下发的权限")
                # 用户不具备一键下发的权限
                msg = "[操作{}成功]\n用户:{}\n设备:{}\n状态:流程已创建待审批\n==========".format(
                    kwargs['task'], kwargs['commit_user'], kwargs['device'])
                send_msg_sec_manage(msg)
                return
            # 判断用户是否属于 engine_master 组或者超级用户，该组被指定用于一键下发配置的权限
            elif flow_record.origin == '运维平台':
                if _user.groups.filter(name='engine_master').exists() or _user.is_superuser:
                    # if _user.groups.filter(name='engine_master').exists():
                    # 批准执行
                    netmiko_error = ''
                    netconf_error = ''
                    path = ''
    ...
    def get_h3c_service_obj(self):
        try:
            ...
            send_msg_sec_manage(
                "[安全纳管]华三防火墙 {} 获取华三单个设备服务组对象列表失败:{}".format(self.host, str(e)))
            return False
                ...
                try:
                    ...
                    if dev_info['category__name'] == '防火墙':
                        try:
                            ...
                            device.closed()
                            # send_msg_sec_manage(
                            #     "运营平台操作华三防火墙 {} 拼接地址组对象数据失败:{}".format(host, str(e)))
                except Exception as e:
            ...
            if 'netconf' in dev_infos[0]['protocol']:
                ...
                if dev_info['device_type'] == 'huawei_usg':
                    ...
                    try:
                        ...
                        # send_msg_sec_manage(
                        #     "运营平台操作华为防火墙 设备 {} 服务组组对象采集异常：{}".format(host, str(e)))
                        return False
                    device.closed()
        return False
                ...
                try:
                    ...
                    if dev_info['category__name'] == '防火墙':
                        try:
                            ...
                            # print(traceback.print_exc())
                            # send_msg_sec_manage(
                            #     "运营平台操作华三防火墙 {} 获取服务组对象数据失败:{}".format(host, str(e)))
                except Exception as e:
            ...
            if 'netconf' in dev_infos[0]['protocol']:
                ...
                if dev_info['device_type'] == 'huawei_usg':
                    ...
                    try:
                        ...
                        # send_msg_sec_manage(
                        #     "运营平台操作华为防火墙 设备 {} 获取服务组组对象异常：{}".format(host, str(e)))
                        device.closed()
                        return False
    ...
    if device['vendor__name'] == '山石网科':
        try:
            ...
            if isinstance(src_zone_object, dict):
                ...
                cmds += ['no src-zone']
                send_ws_msg(group_name=room_group_name,
                            data=dict(device=device, result='OK', content='源安全域识别Any……OK!'))
            ...
            if manual_dst_addr:
                ...
                send_ws_msg(group_name=room_group_name,
                            data=dict(device=device, result='OK', content='自定义目的地址识别……OK!'))
            else:
                if old_dst_ip:
            ...
            if isinstance(final_paths, list):
                ...
                if final_paths:
                    ...
                    return
            send_ws_msg(group_name=room_group_name,
                        data=dict(device=device, result='Failed', content='编辑策略异常, 进程退出'))
        except Exception as e:
            print(str(e))
            # print(traceback.print_exc())
