...
from apps.dcs_control.db import (
    cmdb_mongo as _cmdb_mongo,
    dnat_mongo as _dnat_mongo,
    snat_mongo as _snat_mongo,
    address_mongo as _address_mongo,
    service_mongo as _service_mongo,
    servgroup_mongo as _servgroup_mongo,
    predefined_mongo as _predefined_mongo,
    SwitchFailBackDB as _SwitchFailBackDB,
    SwitchFailBackTaskDB as _SwitchFailBackTaskDB,
)

cmdb_mongo = _cmdb_mongo
dnat_mongo = _dnat_mongo
snat_mongo = _snat_mongo
address_mongo = _address_mongo
service_mongo = _service_mongo
servgroup_mongo = _servgroup_mongo
# 系统预定义服务
predefined_mongo = _predefined_mongo
SwitchFailBackDB = _SwitchFailBackDB
SwitchFailBackTaskDB = _SwitchFailBackTaskDB
if DEBUG:
    CELERY_QUEUE = "dev"
else:
    CELERY_QUEUE = "config"
...
def send_ws_msg(group_name, data):
    ...
    async_to_sync(_channel_layer.group_send)(
        group_name, {"type": "sec_device_message", "message": data}
    )
    return
...
def sec_callback(flow_record_id):
    """
    流程完成后的外部回调通知。
    回调地址由接口调用方在请求体中携带 callback_url，若未携带则不做回调。
    """
    ...
    payload = _to_json_serializable(dict(flow_record))
    try:
        response = requests.post(callback_url, json=payload, timeout=10)
        response.raise_for_status()
        log.info(
            "sec_callback 成功: flow_id=%s, status=%s",
            flow_record_id,
...
def run_cmd_config(*cmds, **dev_info):
    """
    原子锁保护的 SSH 命令下发。
    使用 cache.add 实现原子设备锁，避免多任务并发下同一设备重复操作。
    :param cmds: 要下发的命令列表
    :param dev_info: 设备登录信息（包含 ip 等字段）
    :return: (res: bool, path: str, error: str)
    """
    try:
...
def run_netconf_config(**dev_info):
    """
    原子锁保护的 Netconf 配置下发。
    使用 device_lock 上下文管理器，避免多任务并发下同一设备重复操作。
    :param dev_info: 设备登录信息（包含 ip、port、device_type、class_method、cmd 等字段）
    :return: (res: bool, info: str)
    """
    try:
        with device_lock(dev_info["ip"]):
            log.info(
                "开始执行 Netconf 配置下发: ip=%s, method=%s",
                dev_info["ip"],
                dev_info.get("class_method"),
            )
            try:
                if dev_info["port"] == 830 and dev_info["device_type"] in [
                    "h3c",
                    "hpcomware",
                ]:
                    device = H3CSecPath(
                        ...
                        timeout=600,
                        device_params=dev_info["device_type"],
                    )
                    ...
                    if class_method:
                        ...
                        if isinstance(dev_info["cmd"], list):
                            ...
                            for _cmd in dev_info["cmd"]:
                                ...
                                log.debug(
                                    "run_netconf_config (分段) _res=%s, _info=%s",
                                    _res,
                                    _info,
                                )
                                res = res and _res
                            ...
                            log.debug(
                                "run_netconf_config (H3C单段) res=%s, info=%s",
                                res,
                                info,
                            )
                            device.closed()
                    ...
                    dev_info["port"] == 830 and dev_info["device_type"] == "huawei_usg"
                ):
                    device = HuaweiUSG(
                        host=dev_info["ip"],
                        user=dev_info["username"],
                    ...
                    if class_method:
                        ...
                        res, info = class_method(device, dev_info["cmd"])
                        log.debug(
                            "run_netconf_config (Huawei) res=%s, info=%s", res, info
                        )
                        device.closed()
                        return res, info
                else:
                ...
                log.error("run_netconf_config 执行异常: %s", traceback.format_exc())
...
def get_firewall_zone(**kwargs):
    ...
    hostip = kwargs["hostip"]
    if vendor == "Hillstone":
        res = MongoOps(db="Automation", coll="hillstone_zone").find(
...
class SecFirewallMain:
        ...
        if isinstance(self.dev_infos, list) and len(self.dev_infos) > 0:
            ...
            if "telnet" in self.dev_infos["protocol"]:
                self.dev_info = {
                    ...
                    "encoding": "utf-8",
                    "config_mode_command": self.config_mode_command_map[
                        self.dev_infos["vendor__alias"]
                    ],
                }
            else:
                self.dev_info = {
                    "device_type": self.device_type_map[
                        self.dev_infos["vendor__alias"]
                    ],
                    "ip": host,
    ...
    def flow_engine(self, *args, **kwargs):
        ...
        if res:
            ...
            if kwargs.get("task") == AutoFlowTasks.DNAT:
                ttp_res = HillstoneFsm.config_dnat(path)
                if not isinstance(ttp_res, dict):
                    ttp_method = HillstoneFsm.get_map("standard")
                    ttp_res = ttp_method(path=path) if ttp_method else None
            else:
                ttp_method = HillstoneFsm.get_map("standard")
                ttp_res = ttp_method(path=path) if ttp_method else None
            if ttp_res:
...
class FirewallMain(object):
    def __init__(self, host):
        ...
        if isinstance(self.dev_infos, list) and len(self.dev_infos) > 0:
            ...
            if "netconf" in self.dev_infos["protocol"]:
                if (
                    self.dev_infos["vendor__alias"] == "H3C"
                    and self.dev_infos["category__name"] == "防火墙"
                ):
                    self.dev_info = {
                        "device_type": "h3c",
                        "ip": (
                            self.dev_infos.get("bind_ip__ipaddr")
                            if self.dev_infos.get("bind_ip__ipaddr")
                            else host
                        ),
                    ...
                    self.dev_infos["vendor__alias"] == "Huawei"
                    and self.dev_infos["category__name"] == "防火墙"
                ):
                    self.dev_info = {
    ...
    def flow_engine(self, *args, **kwargs):
        ...
        if res:
            ...
            if class_method == "Hillstone" or path.endswith(".txt"):
                if kwargs.get("task") == AutoFlowTasks.DNAT:
                    # DNAT 下发后解析结果，提取 rule ID 并回填到 flow_record.ttp
                    ttp_res = HillstoneFsm.config_dnat(path)
                    if not isinstance(ttp_res, dict):
                        ttp_method = HillstoneFsm.get_map("standard")
                        ttp_res = ttp_method(path=path) if ttp_method else None
    ...
    def hillstone_address_detail(self, **kwargs):
        ...
        if kwargs.get("add_detail_ip"):
            ...
            self.back_off_cmds += [
                "no range " + kwargs["range_start"] + " " + kwargs["range_end"]
            ]
        elif kwargs.get("add_detail_exclude_range"):
            self.cmds += [name]
            self.back_off_cmds += [name]
            self.cmds += [
                "exclude range " + kwargs["range_start"] + " " + kwargs["range_end"]
            ]
            self.back_off_cmds += [
                "no exclude range " + kwargs["range_start"] + " " + kwargs["range_end"]
            ]
            ...
            self.back_off_cmds += [
                "range " + kwargs["range_start"] + " " + kwargs["range_end"]
            ]
            ...
            res = address_mongo.find(
                query_dict=dict(hostip=kwargs["hostip"], name=kwargs["name"]),
                fields={"_id": 0},
            )
            if not res:
                raise RuntimeError(
                    "hillstone_address_detail" + " 未获取到现有的配置，无法生成回退命令"
                )
            for i in res:
                if i.get("ip"):
                    if i["ip"]:
    ...
    def hillstone_slb_detail(self, **kwargs):
        ...
        # 删除对象
            ...
            if isinstance(paths, list):
                before_ttp_res = HillstoneFsm.check_slb_config_before(
                    slb_name=slb_name, path=paths[0]
                )
                if "before_config" in before_ttp_res.keys():
                    self.back_off_cmds += [
                        "slb-server-pool " + slb_name
                    ] + before_ttp_res["before_config"]
        ...
        # TODO(jmli12):编辑对象 #生成配置，再根据配置生成回退命令有待验证
            ...
            if isinstance(paths, list):
                ...
                if "before_config" in before_ttp_res.keys():
                    ...
                    _HillstoneBase = HillstoneBase(
                        self.cmds, self.dev_info, self.dev_infos
                    )
                    config_cmds = _HillstoneBase.config_slb_pool(**kwargs)
                    if config_cmds:
                        self.cmds += config_cmds
    ...
    def h3c_address_detail(self, **kwargs):
        ...
        if kwargs["method"] == "create":
            ...
            device = H3CSecPath(
                host=self.dev_info["ip"],
                user=self.dev_info["username"],
                password=self.dev_info["password"],
                timeout=600,
                device_params=self.dev_info["device_type"],
            )
            if kwargs.get("del_detail_ip"):
                for _tmp in kwargs["ip_mask"]:
                    ...
                    try:
                        ...
                        if _tmp_id:
                            oms["IPv4Objs"]["Obj"].append(
                                {"Group": kwargs["name"], "ID": str(_tmp_id[0]["ID"])}
                            )
                            back_oms["IPv4Objs"]["Obj"].append(_tmp_id[0])
                        ...
                        try:
                            ...
                            raise ValueError(
                                "h3c_address_detail执行del_detail_ip错误，地址对象中不包含该条目"
                            )
                    # else:
                ...
                for _tmp in kwargs["hostipv4addr"]:
                    _tmp_ip = IPAddress(_tmp)  # demo: 10.254.0.1/24
                    _tmp_id = device.get_ipv4_objs(
                        name=kwargs["name"], HostIPv4Address=_tmp_ip.format()
                    )
                    if _tmp_id:
                        oms["IPv4Objs"]["Obj"].append(
                            {"Group": kwargs["name"], "ID": str(_tmp_id[0]["ID"])}
            ...
            if kwargs.get("add_object"):
                ...
                oms = [
                    {
                        "@xmlns": "http://www.h3c.com/netconf/config:1.0",
                        "@xc:operation": "create",
                        "IPv4Groups": {"Group": {"Name": kwargs["name"]}},
                    },
                    {
                        "@xmlns": "http://www.h3c.com/netconf/config:1.0",
                        "@xc:operation": "create",
                ...
                OMS = {
                    "IPv4Groups": {
                        "Group": {"Name": kwargs["name"], "Description": None}
                    },
                    "IPv4Objs": {
                        "Obj": {
                            "Group": kwargs["name"],
    ...
    def huawei_address_detail(self, **kwargs):
        ...
        if kwargs.get("add_detail_ip"):
            ...
            for _cmd in kwargs["ip_mask"]:
                ...
                tmp_ip = (
                    _tmp_ip.network.format() + "/" + str(_tmp_ip.netmask.netmask_bits())
                )
                ...
                oms["elements"].append(
                    {
                        ...
                        "address-ipv4": _tmp_ip.network.format()
                        + "/"
                        + str(_tmp_ip.netmask.netmask_bits()),
                    }
                )
            ...
            for elem in oms["elements"]:
                if (
                    elem.get("start-ipv4") == kwargs["range_start"]
                    and elem.get("end-ipv4") == kwargs["range_end"]
                ):
                    raise RuntimeError(
                        "安全纳管-华为-地址组-增加range条目，不允许重复条目"
                    )
            # 继续执行拼接
            count_flag += 1
            oms["elements"].append(
    ...
    def hillstone_service_detail(self, **kwargs):
            ...
            for _ser in kwargs["objects"]:
                ...
                if _ser.get("add_detail"):
                    if _ser["protocol"] == "ICMP":
                        ...
                        self.cmds += [
                            f"{_ser['protocol'].lower()} dst-port {_ser['start_dst_port']} "
                            f"{_ser['end_dst_port']}"
                        ]
                        self.back_off_cmds += [
                            f"no {_ser['protocol'].lower()} dst-port {_ser['start_dst_port']} "
                            f"{_ser['end_dst_port']}"
                        ]
                    else:
                        self.cmds += [
        ...
        # 删除
            ...
            res = service_mongo.find(
                query_dict=dict(hostip=kwargs["hostip"], name=kwargs["name"]),
                fields={"_id": 0},
            )
            if not res:
                raise RuntimeError(
                    "hillstone_service_detail" + " 未获取到现有的配置，无法生成回退命令"
                )
            for i in res[0]["items"]:
                if all(
                    ...
                    self.back_off_cmds += [
                        f"{i['protocol']} dst-port {i['dst-port-min']} {i['dst-port-max']} "
                        f"src-port {i['src-port-min']} {i['src-port-max']}"
                    ]
                elif all(
                    k in i for k in ("src-port-min", "dst-port-max", "dst-port-min")
                ):
                    self.back_off_cmds += [
                        f"{i['protocol']} dst-port {i['dst-port-min']} {i['dst-port-max']} "
                    ...
                    self.back_off_cmds += [
                        f"{i['protocol']} dst-port {i['dst-port-min']}"
                    ]
        else:
            raise RuntimeError(
                "FirewallMain.hillstone_service_detail 未匹配到method参数"
            )
        return self.cmds, self.back_off_cmds
    ...
    def h3c_service_detail(self, **kwargs):
            ...
            oms = [
                {
                    "@xmlns": "http://www.h3c.com/netconf/config:1.0",
                    "@xc:operation": "create",
                    "ServGroups": {"Group": {"Name": kwargs["name"]}},
                },
                {
                    "@xmlns": "http://www.h3c.com/netconf/config:1.0",
                    "@xc:operation": "create",
        ...
        # 删除
            ...
            OMS = {
                "ServGroups": {"Group": {"Name": kwargs["name"], "Description": None}},
                "ServObjs": {
                    "Obj": {
                        "Group": kwargs["name"],
                        "ID": None,
    ...
    def huawei_service_detail(self, **kwargs):
            ...
            for _ser in kwargs["objects"]:
                ...
                if _ser.get("add_detail"):
                    if _ser["protocol"] in ["TCP", "UDP"]:
                        Obj.append(
                            {
                                ...
                                _ser["protocol"].lower(): {
                                    "source-port": {
                                        "start": str(_ser["start_src_port"]),
                                        "end": str(_ser["end_src_port"]),
                                    },
                                    "dest-port": {
                                        "start": str(_ser["start_dst_port"]),
                        ...
                        Obj.append({"id": str(count_flag), "icmp-name": "echo"})
                        count_flag += 1
                        Obj.append({"id": str(count_flag), "icmp-name": "echo-reply"})
                    count_flag += 1
            if Obj:
                oms = {
                    "@nc:operation": "create",
        ...
        # 编辑
            ...
            delObj = []
            count_flag = (
                1  # elements id填充计数器 次数设为1 方便后面max取当前最大再相加
            )
            device = HuaweiUSG(
                host=self.dev_info["ip"],
                user=self.dev_info["username"],
            ...
            if _service_req:
                ...
                for _ser in kwargs["objects"]:
                    ...
                    if _ser.get("add_detail"):
                        ...
                        if _ser["protocol"] in ["TCP", "UDP"]:
                            ...
                            addObj.append({"id": str(count_flag), "icmp-name": "echo"})
                            count_flag += 1
                            addObj.append(
                                {"id": str(count_flag), "icmp-name": "echo-reply"}
                            )
                    # 删除条目
                    if _ser.get("del_detail"):
                        if _ser.get("ID"):
                ...
                raise RuntimeError(
                    "FirewallMain.huawei_service_detail 没能获取到待编辑的对象"
                )
        # 删除
        elif kwargs.get("del_object"):
            device = HuaweiUSG(
            ...
            if _service_req:
                ...
                raise RuntimeError(
                    "FirewallMain.huawei_service_detail 没能获取到待删除的对象"
                )
    ...
    def hillstone_dnat_detail(self, **kwargs):
        ...
        if self.dev_infos["soft_version"].find("5.5R6") != -1:
            ...
            self.dev_infos["soft_version"].find("Version 5.0 SG6000-M-2-5.0R3P9.bin")
            != -1
        ):
        ...
        if kwargs.get("add_object"):
            ...
            if "custom_slb" in trans_to.keys():
                ...
                # 去除空字符串命令
                self.cmds += [
                    x.lstrip() for x in tmp_commands.split("\n") if len(x.strip()) != 0
                ]
            # 直接引用服务名
            if "name" in service.keys():
                ...
                for _sub_ser in service["multi"]:
                    if all(
                        k in _sub_ser for k in ("protocol", "start_port", "end_port")
                    ):
            ...
            # 创建服务对象 判断系统预定义服务和已经定义的服务
                ...
                _query_service = MongoNetOps.hillstone_service_query(
                    hostip=self.dev_info["ip"],
                ...
                if _query_service:
                    if "items" in _query_service.keys():
                        ...
                        if len(_query_service["items"]) == 1:
                            log.debug(
                                "服务对象匹配，引用现有服务: %s", _query_service["name"]
                            )
                            _service = _query_service["name"]
                if _service is None:
                    # 服务名
                    _service = "auto_{}_{}_{}".format(
                        str(service["start_port"]),
                        str(service["end_port"]),
                        str(int(time.time())),
                    )
                    self.cmds += [f"service {_service}"]
                    self.back_off_cmds += [f"no service {_service}"]
            ...
            if (
                all(k in service for k in ("protocol", "start_port", "end_port"))
                and kwargs.get("port") is not None
            ):
                if service["start_port"] != service["end_port"]:
                    raise RuntimeError(
                        "山石网科，公网发布，不允许公网服务端口是范围且同时指定内网端口的情况"
                    )
            ...
            _vars = {
                "id": kwargs.get(
                    "id"
                ),  # 如果用户不指定，系绝会为觃则自劢生成一个 ID。如果指定的ID 为已有的 DNAT 规则的 ID，已有的规则会被覆盖。
                "ingress_interface": kwargs.get(
                    "ingress_interface"
                ),  # 指定匹配该 dnat 规则的入接口
                "insert": kwargs.get("insert"),  # [before id | after id | top]
                "from": kwargs.get(
                    "from"
                ),  # {'address_book': 'address-book'} | {'ip':'A.B.C.D '} | {'any': True}
        ...
        # 编辑DNAT规则
            ...
            _vars = {
                ...
                "from": kwargs.get(
                    "from"
                ),  # {'address-book': 'address-book'} | {'ip':'A.B.C.D/M '} | {'any': True}
                "to": kwargs[
                    "to"
                ],  # {'object': 'address-book'} | {'ip':'A.B.C.D/M' A.B.C.D}
            ...
            if isinstance(paths, list):
                before_ttp_res = HillstoneFsm.check_dnat_config_before(
                    rule_id=kwargs["id"], path=paths[0]
                )
                if "command" in before_ttp_res.keys():
                    ...
                    raise RuntimeError(
                        "hillstone_dnat编辑对象失败:未能生成回退命令,ttp解析失败"
                    )
        # 删除DNAT规则
        elif kwargs.get("del_object"):
            self.cmds += ["ip vrouter trust-vr"]
            ...
            if isinstance(paths, list):
                ...
                if "command" in before_ttp_res.keys():
                    ...
                    raise RuntimeError(
                        "hillstone_dnat删除对象失败:未能生成回退命令,ttp解析失败"
                    )
            return self.cmds, self.back_off_cmds
        # 移动DNAT规则
            ...
            log.debug("hillstone_snat sort_object _vars: %s", _vars)
    ...
    def hillstone_snat_detail(self, **kwargs):
        ...
        if kwargs.get("add_object"):
            ...
            # 创建服务对象 判断系统预定义服务和已经定义的服务
                ...
                raise ValueError(
                    "service不符合格式，山石service字段只支持name和定义服务对象"
                )
            # 不允许公网服务端口是范围且同时指定内网端口的情况
            if (
                all(k in service for k in ("protocol", "start_port", "end_port"))
            ...
            _vars = {
                ...
                "ingress_interface": kwargs["ingress"][
                    "name"
                ],  # 指定匹配该 snat 规则的入接口
                "insert": kwargs.get("insert", ""),  # [before id | after id | top]
                "from": kwargs.get(
                    "from", ""
                ),  # {'object': 'address-book'} | {'ip':'A.B.C.D '} | {'any': True}
                "to": kwargs.get(
                    "to", ""
                ),  # {'object': 'address-book'} | {'ip':'A.B.C.D'}
                "service": _service,  # 服务对象名
                "trans_to": kwargs.get("trans_to", ""),
                "egress_interface": kwargs["egress"]["name"],
                "nat_mode": kwargs.get(
                    "nat_mode"
                ),  # static|dynamicip|dynamicport[sticky]
                "log": kwargs.get("log"),  # log  # 是否启用，boolean
                "soft_version": soft_version,  # 版本区分
                "track_tcp": kwargs.get("track_tcp"),  # tcp 监测
                "track_ping": kwargs.get("track_ping"),  # ping 监测
        ...
        # 编辑SNAT规则
            ...
            paths = BatManMain.hillstone_send_cmds(*before_cmds, **self.dev_info)
            if isinstance(paths, list):
                before_ttp_res = HillstoneFsm.check_snat_config_before(
                    rule_id=kwargs["id"], path=paths[0]
                )
                if "command" in before_ttp_res.keys():
                    self.back_off_cmds += [before_ttp_res["command"]]
        ...
        # 删除SNAT规则
            ...
            if isinstance(paths, list):
                ...
                if "command" in before_ttp_res.keys():
                    ...
                    raise RuntimeError(
                        "hillstone_snat删除对象失败:未能生成回退命令,ttp解析失败"
                    )
        # 移动SNAT规则
        elif kwargs.get("sort_object"):
            self.cmds += ["ip vrouter trust-vr"]
    ...
    def hillstone_sec_policy_detail(self, **kwargs):
        ...
        if kwargs.get("add_object"):
            ...
            if kwargs.get("id") is not None:
                _vars["id"] = kwargs[
                    "id"
                ]  # 如果不指定，系绝会为觃则自劢生成一个 ID。如果指定的ID为已有的规则的ID，已有的规则会被覆盖。
            ...
            tmp_commands = tmp_commands.split("\n")
            log.debug(
                "hillstone_sec_policy move_snat cmds: %s",
                [x.strip() for x in tmp_commands],
            )
            self.cmds += [x.strip() for x in tmp_commands]
            before_cmds = ["show configuration policy"]
            paths = BatManMain.send_cmds(*before_cmds, **self.dev_info)
            if isinstance(paths, list):
                before_ttp_res = HillstoneFsm.check_sec_policy_config_before(
                    rule_id=kwargs["id"], path=paths[0]
                )
    ...
    def h3c_dnat_detail(self, **kwargs):
        ...
        if kwargs.get("add_object"):
                ...
                oms += [
                    {
                        ...
                        "OMS": {
                            "@xmlns": "http://www.h3c.com/netconf/config:1.0",
                            "@xc:operation": "create",
                            "ServGroups": {"Group": {"Name": _service_name}},
                        },
                    },
                    {
                        "@xmlns": "http://www.h3c.com/netconf/config:1.0",
            ...
            # 创建服务对象 判断系统预定义服务和已经定义的服务
                ...
                raise RuntimeError(
                    "service不符合格式，key name not match name or protocol"
                )
            ...
            if "ip" in trans_to.keys():
                ...
                raise RuntimeError(
                    "trans_to不符合格式，华三trans_to字段只支持ip，不支持地址对象和SLB"
                )
            ...
            config_data = [
                {"GlobalPolicyRules": {"Rule": {"RuleName": name}}},
                {"GlobalPolicyRuleMembers": GlobalPolicyRuleMembers},
                {"GlobalPolicyRuleDstObj": {"Rule": GlobalPolicyRuleDstObj}},
                {
            ...
            back_oms += [
                {
                    "@xmlns": "http://www.h3c.com/netconf/config:1.0",
                    "@xc:operation": "remove",
                    "NAT": {"GlobalPolicyRules": {"Rule": {"RuleName": name}}},
                }
            ]
            return oms, back_oms
        # 编辑DNAT规则
            ...
            if len(before) > 0:
                ...
                raise RuntimeError(
                    "h3c_dnat_detail，编辑DNAT策略，未获取到待编辑的策略"
                )
        # 删除DNAT规则
        elif kwargs.get("del_object"):
            device = H3CSecPath(
    ...
    def h3c_snat_detail(self, **kwargs):
        ...
        if kwargs.get("add_object"):
            ...
            PolicyRuleMemberSrcObj = {
                "@xc:operation": "merge",
                "Rule": {"RuleName": name, "SrcObjGrpList": {}},
            }
            if "object" in _from.keys():
                PolicyRuleMemberSrcObj["Rule"]["SrcObjGrpList"]["SrcIpObj"] = _from[
                    "object"
                ]
            ...
            PolicyRuleMemberDstObj = {
                "@xc:operation": "merge",
                "Rule": {"RuleName": name, "DstObjGrpList": {}},
            }
            if "object" in _to.keys():
                PolicyRuleMemberDstObj["Rule"]["DstObjGrpList"]["DstIpObj"] = _to[
                    "object"
                ]
            ...
            PolicyRuleMemberSrvObj = {
                "@xc:operation": "merge",
                "Rule": {"RuleName": name, "SrvObjGrpList": {}},
            }
            if "name" in service.keys():
                PolicyRuleMemberSrvObj["Rule"]["SrvObjGrpList"]["SrvObj"] = service[
                    "name"
                ]  # 服务对象名
            ...
            config_data = [
                {"PolicyRules": {"Rule": {"RuleName": name}}},
            ...
            self.back_off_cmds += [
                {
                    "@xmlns": "http://www.h3c.com/netconf/config:1.0",
                    "@xc:operation": "remove",
                    "NAT": {"PolicyRules": {"Rule": {"RuleName": name}}},
                }
            ]
            return self.cmds, self.back_off_cmds
            ...
            if service_multi:
                ...
                for _manual_service in service_multi:
                    if _manual_service["protocol"].lower() in ["tcp", "udp"]:
                        ...
                        _tmp_obj = {"Type": type_map[_manual_service["type"]]}
                        ServObjItem.append(json.dumps(_tmp_obj))
                IPv4ServObj = {
                    "IPv4ServObj": {
                        "@xc:operation": "merge",
                ...
                IPv4SrcSimpleAddr = {
                    "IPv4SrcSimpleAddr": {
                        ...
                        "SrcSimpleAddr": {
                            ...
                            "IsIncrement": False,
                            "SimpleAddrList": [
                                {"SimpleAddrItem": [x for x in from_addr]}
                            ],
                        },
                    }
                }
                ...
                IPv4DestSimpleAddr = {
                    "IPv4DestSimpleAddr": {
                        ...
                        "DestSimpleAddr": {
                            ...
                            "IsIncrement": False,
                            "SimpleAddrList": [
                                {"SimpleAddrItem": [x for x in to_addr]}
                            ],
                        },
                    }
                }
            ...
            if "DestZoneList" in rules.keys():
                BackIPv4DestSecZone = {
                    ...
                    "DestSecZone": {
                        ...
                        "IsIncrement": False,
                        "NameList": [
                            {"NameItem": rules["DestZoneList"]["DestZoneItem"]}
                        ],
                    },
                }
                IPv4DestSecZone = {
                ...
                BackIPv4SrcAddr = {
                    "IPv4SrcAddr": {
                        ...
                        "SrcAddr": {
                            ...
                            "IsIncrement": False,
                            "NameList": [
                                {"NameItem": rules["SrcAddrList"]["SrcAddrItem"]}
                            ],
                        },
                    }
                }
                ...
                BackIPv4DestAddr = {
                    "IPv4DestAddr": {
                        ...
                        "DestAddr": {
                            ...
                            "IsIncrement": False,
                            "NameList": [
                                {"NameItem": rules["DestAddrList"]["DestAddrItem"]}
                            ],
                        },
                    }
                }
                ...
                BackIPv4SrcSimpleAddr = {
                    "IPv4SrcSimpleAddr": {
                        ...
                        "SrcSimpleAddr": {
                            ...
                            "SimpleAddrList": [
                                {
                                    "SimpleAddrItem": rules["SrcSimpleAddrList"][
                                        "SrcSimpleAddrItem"
                                    ]
                                }
                            ],
                ...
                BackIPv4DestSimpleAddr = {
                    "IPv4DestSimpleAddr": {
                        ...
                        "DestSimpleAddr": {
                            ...
                            "SimpleAddrList": [
                                {
                                    "SimpleAddrItem": rules["DestSimpleAddrList"][
                                        "DestSimpleAddrItem"
                                    ]
                                }
                            ],
                ...
                BackIPv4ServGrp = {
                    "IPv4ServGrp": {
                        ...
                        "ServGrp": {
                            ...
                            "IsIncrement": False,
                            "NameList": {
                                "NameItem": rules["ServGrpList"]["ServGrpItem"]
                            },
                        },
                    }
                }
                ...
                BackIPv4ServObj = {
                    "IPv4ServObj": {
                        ...
                        "ServObj": {
                            ...
                            "IsIncrement": False,
                            "ServObjList": {
                                "ServObjItem": rules["ServObjList"]["ServObjItem"]
                            },
                        },
                    }
                }
            ...
            self.back_off_cmds = [
                {
                    "top": {
                        "@xmlns": "http://www.h3c.com/netconf/action:1.0",
                        "SecurityPolicies": {
                            "MoveIPv4Rule": {"Rule": {"ID": rule_id, "MoveType": 4}}
                        },
                    }
                }
            ]
    ...
    def huawei_dnat_detail(self, **kwargs):
        ...
        if kwargs.get("add_object"):
            ...
            if "protocol" in service.keys():
                if "ip" in trans_to.keys():
                    ...
                    raise RuntimeError(
                        "trans_to不符合格式，华为trans_to字段只支持ip，不支持地址对象和SLB"
                    )
                if "ip" in to.keys():
                    global_ip = to["ip"]
                else:
                    raise ValueError(
                        "to不符合格式，华为to字段只支持ip，不支持地址对象和SLB"
                    )
                ...
                if protocol == "1":
                    ...
                    if service["start_port"] != service["end_port"]:
                        for _sub_port in range(
                            service["start_port"], service["end_port"]
                        ):
                            _sub_name = (
                                "{name}_{protocol}_{start_port}_{end_port}".format(
                                    name=name,
                        ...
                        oms.append(
                            {
                                ...
                                "global": {"start-ip": global_ip},
                                "global-port": {
                                    "start-port": str(service["start_port"])
                                },
                                "inside": {"start-ip": TransDstIP},
                ...
                for _sub_ser in service["multi"]:
                    ...
                    if protocol == "1":
                        ...
                        if _sub_ser["start_port"] != _sub_ser["end_port"]:
                            for _sub_port in range(
                                _sub_ser["start_port"], _sub_ser["end_port"]
                            ):
                                _sub_name = (
                                    "{name}_{protocol}_{start_port}_{end_port}".format(
                                        name=name,
                            ...
                            oms.append(
                                {
                                    ...
                                    "global": {"start-ip": global_ip},
                                    "global-port": {
                                        "start-port": str(_sub_ser["start_port"])
                                    },
                                    "inside": {"start-ip": TransDstIP},
                                    "inside-port": {
                                        "start-port": (
                ...
                raise ValueError(
                    "service不符合格式，华为只支持自定义协议和端口，不支持引用服务对象"
                )
        ...
        # 编辑DNAT规则
            ...
            if _dnat_req:
                ...
                raise RuntimeError(
                    "FirewallMain.huawei_dnat_detail 没能获取到待编辑的对象"
                )
            ...
            if "protocol" in service.keys():
                ...
                if protocol == "1":
                    ...
                    if "global_port" in service.keys():
                        ...
                        raise RuntimeError(
                            "global_port不符合格式，华为DNAT配置TCP/UDP需要global_port和protocol"
                        )
                    oms = {
                        "@nc:operation": "replace",
                        "name": name,
                ...
                raise ValueError(
                    "FirewallMain.huawei_dnat_detail service不符合格式，华为只支持自定义协议和端口，不支持引用服务对象"
                )
        # 删除DANT规则
        elif kwargs.get("del_object"):
            device = HuaweiUSG(
    ...
    def huawei_snat_detail(self, **kwargs):
        ...
        if kwargs.get("add_object"):
            ...
            if "protocol" in service.keys():
                if service["start_port"] == service["end_port"]:
                    config_obj["service"] = {
                        service["protocol"].lower: {"dest-port": service["start_port"]}
                    }
                else:
                    config_obj["service"] = {
    ...
    def huawei_sec_policy(self, **kwargs):
        name = kwargs["name"]
        action_map = {"deny": "false", "permit": "true"}
        if "add_object" in kwargs.keys() or "edit_object" in kwargs.keys():
            ...
            if from_addr:
                for _sub_from_addr in from_addr:
                    ...
                    if _sub_from_addr.find("-") != -1:
                        if (
                            "address-ipv4-range"
                            not in policy_data["rule"]["source-ip"].keys()
                        ):
                        ...
                        if (
                            ...
                            policy_data["rule"]["source-ip"]["address-ipv4"] = [
                                _sub_from_addr
                            ]
                        else:
                            policy_data["rule"]["source-ip"]["address-ipv4"] += [
            ...
            if to_addr:
                for _sub_to_addr in to_addr:
                    ...
                    if _sub_to_addr.find("-") != -1:
                        if (
                            "address-ipv4-range"
                            not in policy_data["rule"]["destination-ip"].keys()
                        ):
                            ...
                            policy_data["rule"]["destination-ip"][
                                ...
                                {"start-ipv4": _sub_to_addr, "end-ipv4": _sub_to_addr}
                            ]
                        ...
                        if (
                            ...
                            policy_data["rule"]["destination-ip"]["address-ipv4"] = [
                                _sub_to_addr
                            ]
                        else:
                            policy_data["rule"]["destination-ip"]["address-ipv4"] += [
            ...
            if to_obj and "Any" not in to_obj:
                policy_data["rule"]["destination-ip"]["address-set"] = [
                    x for x in to_obj
                ]
            if service_obj and "Any" not in service_obj:
                if "service-object" not in policy_data["rule"]["service"].keys():
                    policy_data["rule"]["service"]["service-object"] = [
                        x for x in service_obj
                    ]
                else:
                    policy_data["rule"]["service"]["service-object"] += [
                        x for x in service_obj
            ...
            if service_multi:
                ...
                for _m_service in service_multi:
                    if _m_service["protocol"].lower() in ["tcp", "udp"]:
                        if (
                            _m_service["protocol"]
                            not in policy_data["rule"]["service"][
                                "service-items"
                            ].keys()
                        ):
                            policy_data["rule"]["service"]["service-items"][
                                _m_service["protocol"]
                            ] = []
                        policy_data["rule"]["service"]["service-items"][
                            _m_service["type"]
                        ].append(
                            {
                                ...
                                "dest-port": "{} to {}".format(
                                    str(_m_service["start_port"]),
                                    str(_m_service["end_port"]),
                                ),
                            }
                        )
                    elif _m_service["protocol"] == "icmp":
                        policy_data["rule"]["service"]["service-items"]["icmp-item"] = [
                            {"icmp-name": "echo"},
    ...
    def get_h3c_snat(self, **kwargs):
        ...
        try:
            ...
            send_msg_sec_manage(
                "安全纳管华三防火墙 {} NETCONF初始化失败:{}".format(self.host, str(e))
            )
            return False
    ...
    def get_huawei_nat_policy(self, **kwargs):
        ...
        try:
            ...
            send_msg_sec_manage(
                "[安全纳管]操作华为防火墙 设备 {} 获取NAT策略表项异常：{}".format(
                    self.host, str(e)
                )
            )
            device.closed()
    ...
    def get_huawei_address_obj(self, **kwargs):
        ...
        try:
            ...
            # print(traceback.print_exc())
            send_msg_sec_manage(
                "[安全纳管]操作华为防火墙 设备 {} 地址组对象采集异常：{}".format(
                    self.host, str(e)
                )
            )
            device.closed()
    ...
    def get_h3c_service_obj(self, **kwargs):
        try:
            ...
            try:
                ...
                # print(traceback.print_exc())
                send_msg_sec_manage(
                    "[安全纳管]操作华三防火墙 {} 获取服务组对象数据失败:{}".format(
                        self.host, str(e)
                    )
                )
            device.closed()
            ...
            send_msg_sec_manage(
                "[安全纳管]华三防火墙 {} 获取华三单个设备服务组对象列表失败:{}".format(
                    self.host, str(e)
                )
            )
            return False
    ...
    def get_huawei_service_obj(self, **kwargs):
        ...
        try:
            ...
            send_msg_sec_manage(
                "[安全纳管]操作华为防火墙 设备 {} 获取服务组组对象异常：{}".format(
                    self.host, str(e)
                )
            )
            device.closed()
    ...
    def get_huawei_sec_policy(self, **kwargs):
        ...
        try:
            ...
            send_msg_sec_manage(
                "[安全纳管]操作华为防火墙 设备 {} 获取安全策略异常：{}".format(
                    self.host, str(e)
                )
            )
            device.closed()
    ...
    def get_huawei_nat_server(self, **kwargs):
        ...
        try:
            ...
            send_msg_sec_manage(
                "[安全纳管]操作华为防火墙 设备 {} 获取全局DNAT表项异常：{}".format(
                    self.host, str(e)
                )
            )
            device.closed()
    ...
    def get_hillstone_address_obj(self, **kwargs):
        res = MongoOps(db="Automation", coll="hillstone_address").find(
            query_dict=dict(hostip=self.host), fields={"_id": 0}
        )
        return res
    ...
    def get_single_hillstone(host):
        ...
        if dev_infos:
            ...
            if paths and isinstance(paths, list):
                ...
                if isinstance(sec_policy_res, list):
                    ...
                    if sec_policy_result:
                        StandardFSMAnalysis.hillstone_sec_policy(
                            host=host, datas=sec_policy_result
                        )
                ...
                log.warning(
                    "get_single_hillstone 采集失败，paths=%s，host=%s", paths, host
                )
                return False
        return False
        ...
        if dev_infos:
            if "netconf" in dev_infos[0]["protocol"]:
                ...
                try:
                        ...
                        try:
                            ...
                            if sec_policy_res:
                                for i in sec_policy_res:
                                    ...
                                    sec_policy_result.append(i)
                            if sec_policy_result:
                                StandardFSMAnalysis.h3c_secpath_sec_policy(
                                    host, sec_policy_result
                                )
                                return True
                        except Exception as e:
                            device.closed()
                    ...
                    # print(traceback.print_exc())
                    send_msg_sec_manage(
                        "运营平台接口华三防火墙 {} NETCONF初始化失败:{}".format(
                            host, str(e)
                        )
                    )
                    return False
                ...
                if dev_info["device_type"] == "huawei_usg":
                    ...
                    try:
                        ...
                        if sec_policy_res:
                            ...
                            if sec_policy_result:
                                # 格式化落库
                                StandardFSMAnalysis.huawei_usg_sec_policy(
                                    dev_info["manage_ip"], sec_policy_result
                                )
                                # 原始数据格式落库
                                AutomationMongo.insert_table(
                                    db="NETCONF",
                                    hostip=dev_info["manage_ip"],
                                    datas=sec_policy_result,
            ...
            if paths and isinstance(paths, list):
                ...
                try:
                    ...
                    AutomationMongo.failed_log(
                        ip=host, fsm_flag="hillstone", cmd="address", version=str(e)
                    )
                # 服务
                try:
                    ...
                    AutomationMongo.failed_log(
                        ip=host, fsm_flag="hillstone", cmd="service", version=str(e)
                    )
                # 服务组
                try:
                    ...
                    AutomationMongo.failed_log(
                        ip=host, fsm_flag="hillstone", cmd="servgroup", version=str(e)
                    )
                ...
                try:
                    ...
                    if dev_info["category__name"] == "防火墙":
                        try:
                            ...
                            device.closed()
                            send_msg_sec_manage(
                                "运营平台操作华三防火墙 {} 拼接地址组对象数据失败:{}".format(
                                    host, str(e)
                                )
                            )
                except Exception as e:
                    send_msg_sec_manage(
                        "华三防火墙 {} NETCONF初始化失败:{}".format(host, str(e))
                    )
                    return False
        return False
            ...
            if "netconf" in dev_infos[0]["protocol"]:
                ...
                if dev_info["device_type"] == "huawei_usg":
                    ...
                    try:
                        ...
                        send_msg_sec_manage(
                            "运营平台操作华为防火墙 设备 {} 地址组对象采集异常：{}".format(
                                host, str(e)
                            )
                        )
                        return False
            ...
            if "netconf" in dev_infos[0]["protocol"]:
                ...
                if dev_info["device_type"] == "huawei_usg":
                    ...
                    try:
                        ...
                        send_msg_sec_manage(
                            "运营平台操作华为防火墙 设备 {} 服务组组对象采集异常：{}".format(
                                host, str(e)
                            )
                        )
                        return False
                ...
                try:
                    ...
                    if dev_info["category__name"] == "防火墙":
                        try:
                            ...
                            # print(traceback.print_exc())
                            send_msg_sec_manage(
                                "运营平台操作华三防火墙 {} 获取服务组对象数据失败:{}".format(
                                    host, str(e)
                                )
                            )
                except Exception as e:
                    send_msg_sec_manage(
                        "华三防火墙 {} 获取华三单个设备服务组对象列表失败:{}".format(
                            host, str(e)
                        )
                    )
                    return False
            ...
            if "netconf" in dev_infos[0]["protocol"]:
                ...
                if dev_info["device_type"] == "huawei_usg":
                    ...
                    try:
                        ...
                        send_msg_sec_manage(
                            "运营平台操作华为防火墙 设备 {} 获取服务组组对象异常：{}".format(
                                host, str(e)
                            )
                        )
                        device.closed()
        ...
        if dev_infos:
            if "netconf" in dev_infos[0]["protocol"]:
                ...
                try:
                    ...
                    send_msg_sec_manage(
                        "华三防火墙 {} 移动安全策略失败:{}".format(host, str(e))
                    )
                    return False
        return False
        ...
        if dev_infos:
            if "netconf" in dev_infos[0]["protocol"]:
                ...
                try:
                    ...
                    res = _H3cFirewall.delete_sec_policy(
                        hostip=host, rule_id=kwargs["delete_rule_id"]
                    )
                    return res
                except Exception as e:
                    send_msg_sec_manage(
                        "华三防火墙 {} 删除安全策略失败:{}".format(host, str(e))
                    )
                    return False
        return False
        ...
        if dev_infos:
            if "netconf" in dev_infos[0]["protocol"]:
                ...
                try:
                    ...
                    send_msg_sec_manage(
                        "华三防火墙 {} 删除地址对象失败:{}".format(host, str(e))
                    )
                    return False
        return False
        ...
        if dev_infos:
            if "netconf" in dev_infos[0]["protocol"]:
                ...
                try:
                    ...
                    send_msg_sec_manage(
                        "华三防火墙 {} 新建服务对象失败:{}".format(host, str(e))
                    )
                    return False
        return False
        ...
        if dev_infos:
            if "netconf" in dev_infos[0]["protocol"]:
                ...
                try:
                    ...
                    send_msg_sec_manage(
                        "华三防火墙 {} 编辑服务对象失败:{}".format(host, str(e))
                    )
                    return False
        return False
...
def edit_sec_policy(**kwargs):
    log.debug(
        "edit_sec_policy device_object=%s, policy_object=%s",
        kwargs["device_object"],
        kwargs["policy_object"],
    )
    ...
    if device["vendor__name"] == "山石网科":
        try:
            ...
            old_sec_policy = MongoOps(db="Automation", coll="sec_policy").find(
                query_dict=dict(hostip=hostip, id=rule_id), fields={"_id": 0}
            )
            """
            ...
            send_ws_msg(
                group_name=room_group_name,
                data=dict(device=device, result="OK", content="连接成功……"),
            )
            send_ws_msg(
                group_name=room_group_name,
                data=dict(
                    device=device, result="OK", content="开始编辑, rule id " + rule_id
                ),
            )
            ...
            if action != old_sec_policy["action"]:
                ...
                send_ws_msg(
                    ...
                    data=dict(
                        device=device,
                        result="OK",
                        content="策略改变识别……{}……OK!".format(action),
                    ),
                )
            ...
            if manual_service_info:
                ...
                service_data = {
                    "vendor": "hillstone",
                    "create_service_obj": {"name": name, "object": manual_service_info},
                    "hostip": device["manage_ip"],
                    "no_save": True,
                }
                service_path, service_ttp = HillstoneBase.create_service(**service_data)
                log.debug(
                    "create_service 结果: path=%s, ttp=%s", service_path, service_ttp
                )
                send_ws_msg(
                    group_name=room_group_name,
                    data=dict(
                        device=device, result="OK", content="自定义服务识别……OK!"
                    ),
                )
            if service_path:
                service_content = (
                    default_storage.open(service_path).read().decode("utf-8")
                )
                ...
                send_ws_msg(
                    group_name=room_group_name,
                    data=dict(device=device, result="OK", content="新建服务成功……"),
                )
                cmds += ["service " + name]
            ...
            if isinstance(src_zone_object, dict):
                if src_zone_object["name"] != old_sec_policy["src_zone"]:
                    ...
                    send_ws_msg(
                        group_name=room_group_name,
                        data=dict(
                            device=device, result="OK", content="源安全域识别……OK!"
                        ),
                    )
                else:
                ...
                send_ws_msg(
                    group_name=room_group_name,
                    data=dict(
                        device=device, result="OK", content="源安全域识别Any……OK!"
                    ),
                )
            ...
            if isinstance(dst_zone_object, dict):
                if dst_zone_object["name"] != old_sec_policy["dst_zone"]:
                    ...
                    send_ws_msg(
                        group_name=room_group_name,
                        data=dict(
                            device=device, result="OK", content="目的安全域识别……OK!"
                        ),
                    )
                else:
                ...
                send_ws_msg(
                    group_name=room_group_name,
                    data=dict(
                        device=device, result="OK", content="目的安全域识别Any……OK!"
                    ),
                )
            ...
            # 指定地址对象
            log.debug("old_sec_policy src_addr: %s", old_sec_policy["src_addr"])
            # old_src_addr = [x.get('object') for x in old_sec_policy['src_addr']]
            if old_sec_policy["src_addr"]:
                old_src_addr = [
                    item_dict.get(type_name)
                    for item_dict in old_sec_policy["src_addr"]
                    for type_name in item_dict
                    if type_name == "object"
                ]
            else:
                old_src_addr = []
            if (
                src_addr_info
                and isinstance(src_addr_info, list)
                and len(src_addr_info) > 0
            ):
                new_src_addr = [x["name"] for x in src_addr_info]
                log.debug(
                    "src_addr 差异 - 新增: %s, 删除: %s", new_src_addr, old_src_addr
                )
                log.debug(
                    "src_addr 操作 - add: %s, del: %s",
                    cmd_src_addr_add,
                    cmd_src_addr_del,
                )
                if cmd_src_addr_add:
                ...
                insert_mongo_datas["src_addr"] += [
                    {"object": x["name"]} for x in src_addr_info
                ]
                send_ws_msg(
                    group_name=room_group_name,
                    data=dict(
                        device=device, result="OK", content="源地址对象识别……OK!"
                    ),
                )
            else:
                log.debug("old_src_addr（无新地址对象）: %s", old_src_addr)
                if old_src_addr:
            ...
            if old_sec_policy["dst_addr"]:
                old_dst_addr = [
                    item_dict.get(type_name)
                    for item_dict in old_sec_policy["dst_addr"]
                    for type_name in item_dict
                    if type_name == "object"
                ]
            else:
            ...
            if (
                dst_addr_info
                and isinstance(dst_addr_info, list)
                and len(dst_addr_info) > 0
            ):
                new_dst_addr = [x["name"] for x in dst_addr_info]
                # 归集差异比较结果
                cmd_dst_addr_add, cmd_dst_addr_del = HillstoneBase.get_list_diff(
                ...
                insert_mongo_datas["dst_addr"] += [
                    {"object": x["name"]} for x in dst_addr_info
                ]
                send_ws_msg(
                    group_name=room_group_name,
                    data=dict(
                        device=device, result="OK", content="目的地址对象识别……OK!"
                    ),
                )
            else:
                if old_dst_addr:
            ...
            if old_sec_policy["service"]:
                old_ser = [
                    item_dict.get(type_name)
                    for item_dict in old_sec_policy["service"]
                    for type_name in item_dict
                    if type_name == "object"
                ]
            else:
            ...
            if (
                service_info
                and isinstance(service_info, list)
                and len(service_info) > 0
            ):
                ...
                if isinstance(insert_mongo_datas.get("service"), list):
                    insert_mongo_datas["service"] += [
                        {"object": x["name"]} for x in service_info
                    ]
                else:
                    insert_mongo_datas["service"] = [
                        {"object": x["name"]} for x in service_info
                ...
                if old_ser:
                    ...
                    send_ws_msg(
                        group_name=room_group_name,
                        data=dict(
                            device=device, result="OK", content="服务对象识别……OK!"
                        ),
                    )
            # 自定义源地址
            ...
            if manual_src_addr:
                ...
                for src_addr in manual_src_addr:
                    if src_addr["type"] == "subnet":
                        ...
                        new_src_range.append(
                            _content["start_ip"] + " " + _content["end_ip"]
                        )
                        insert_mongo_datas["src_addr"].append(
                            {"range": _content["start_ip"] + " " + _content["end_ip"]}
                        )
                # 归集差异比较结果
                src_ip_add, src_ip_del = HillstoneBase.get_list_diff(
                    old_src_ip, new_src_ip
                )
                log.debug("src_ip 差异 - add: %s, del: %s", src_ip_add, src_ip_del)
                src_range_add, src_range_del = HillstoneBase.get_list_diff(
                    old_src_range, new_src_range
                )
                if src_ip_add:
                ...
                send_ws_msg(
                    group_name=room_group_name,
                    data=dict(
                        device=device, result="OK", content="自定义源地址识别……OK!"
                    ),
                )
            else:
                if old_src_ip:
            ...
            if manual_dst_addr:
                ...
                # 归集差异比较结果
                dst_ip_add, dst_ip_del = HillstoneBase.get_list_diff(
                    old_dst_ip, new_dst_ip
                )
                dst_range_add, dst_range_del = HillstoneBase.get_list_diff(
                    old_dst_range, new_dst_range
                )
                if dst_ip_add:
                    cmds += ["dst-ip " + x for x in dst_ip_add]
                if dst_ip_del:
                    log.debug("dst_ip 待删除: %s", dst_ip_del)
                ...
                send_ws_msg(
                    group_name=room_group_name,
                    data=dict(
                        device=device, result="OK", content="自定义目的地址识别……OK!"
                    ),
                )
            else:
                if old_dst_ip:
            ...
            log.debug("edit_sec_policy 最终 cmds: %s", cmds)
            send_ws_msg(
                group_name=room_group_name,
                data=dict(device=device, result="OK", content="开始下发配置……"),
            )
            ...
            if isinstance(final_paths, list):
                ...
                if final_paths:
                    content = (
                        default_storage.open(final_paths[0]).read().decode("utf-8")
                    )
                    ...
                    send_ws_msg(
                        group_name=room_group_name,
                        data=dict(device=device, result="OK", content="编辑策略成功!"),
                    )
                    MongoOps(db="Automation", coll="sec_policy").delete_many(
                        query=dict(hostip=hostip, id=rule_id)
                    )
                    log.debug("insert_mongo_datas: %s", insert_mongo_datas)
            ...
            send_ws_msg(
                group_name=room_group_name,
                data=dict(
                    device=device, result="Failed", content="编辑策略异常, 进程退出"
                ),
            )
        except Exception as e:
            log.error(
                "edit_sec_policy 山石执行异常: %s\n%s", str(e), traceback.format_exc()
            )
            # print(traceback.print_exc())
        ...
        if src_zone_object:
            if isinstance(src_zone_object, dict):
                ...
                policy_data["rule"]["source-zone"] = [
                    x["name"] for x in src_zone_object
                ]
            else:
        ...
        if dst_zone_object:
            if isinstance(dst_zone_object, dict):
                ...
                policy_data["rule"]["destination-zone"] = [
                    x["name"] for x in dst_zone_object
                ]
            else:
        ...
        send_ws_msg(
            ...
            data=dict(
                device=device,
                result="OK",
                content="策略规则识别……{}……OK!".format(action),
            ),
        )
        if src_addr_info:
            if "address-set" not in policy_data["rule"]["source-ip"].keys():
                #
                policy_data["rule"]["source-ip"]["address-set"] = [
                    x["name"] for x in src_addr_info
                ]
            else:
                policy_data["rule"]["source-ip"]["address-set"] += [
                    x["name"] for x in src_addr_info
        ...
        if manual_src_addr:
            ...
            send_ws_msg(
                group_name=room_group_name,
                data=dict(device=device, result="OK", content="自定义源地址识别……OK!"),
            )
        if manual_dst_addr:
        ...
        if manual_service_info:
            ...
            for _manual_service in manual_service_info:
                if _manual_service["type"] in ["tcp", "udp"]:
                    ...
                    #  },
                    if (
                        _manual_service["type"]
                        not in policy_data["rule"]["service"]["service-items"].keys()
                    ):
                        policy_data["rule"]["service"]["service-items"][
                            _manual_service["type"]
                        ] = []
                    policy_data["rule"]["service"]["service-items"][
                        ...
                        {
                            "source-port": "{} to {}".format(
                                str(_manual_service["src_port_start"]),
                                str(_manual_service["src_port_end"]),
                            ),
                            "dest-port": "{} to {}".format(
                                str(_manual_service["dst_port_start"]),
            ...
            send_ws_msg(
                group_name=room_group_name,
                data=dict(device=device, result="OK", content="自定义服务识别……OK!"),
            )
        log.debug("华为 policy_data: %s", policy_data)
        send_ws_msg(
            group_name=room_group_name,
            data=dict(device=device, result="OK", content="开始编辑安全策略规则……!"),
        )
        log.debug("华为防火墙 manage_ip: %s", device["manage_ip"])
        if is_top:
            res, content = HuaweiUsgSecPolicyConf.config(
                host=device["manage_ip"], rule=policy_data, top=True
            )
        else:
            res, content = HuaweiUsgSecPolicyConf.config(
                host=device["manage_ip"], rule=policy_data, top=False
            )
        log.debug("华为 config_sec_policy 结果: res=%s, content=%s", res, content)
        ...
        _H3cFirewall = H3cFirewall(
            dev_info["ip"], dev_info["username"], dev_info["password"]
        )
        ipv4_rule_res, content = _H3cFirewall.config_ipv4_rule(rule=rule_params)

        log.debug(
            "华三 config_ipv4_rule 结果: res=%s, content=%s", ipv4_rule_res, content
        )
            ...
            if manual_src_addr:
                ...
                data_dict["SecurityPolicies"][0]["IPv4SrcSimpleAddr"] = {
                    ...
                    "SrcSimpleAddr": {
                        ...
                        "SeqNum": None,
                        "IsIncrement": "false",
                        "SimpleAddrList": {"SimpleAddrItem": [x for x in _src_content]},
                    },
                }
                send_ws_msg(
                    group_name=room_group_name,
                    data=dict(
                        device=device, result="OK", content="自定义源地址对象识别……OK!"
                    ),
                )
            if manual_dst_addr:
                _dst_content = []
                if isinstance(manual_dst_addr, list):
                    for _dst_addr in manual_dst_addr:
                        ...
                        log.debug("_dst_content: %s", _dst_content)
                data_dict["SecurityPolicies"][0]["IPv4DestSimpleAddr"] = {
                    ...
                    "DestSimpleAddr": {
                        ...
                        "SeqNum": None,
                        "IsIncrement": "false",
                        "SimpleAddrList": {"SimpleAddrItem": [x for x in _dst_content]},
                    },
                }
                send_ws_msg(
                    group_name=room_group_name,
                ...
                if ServObjList:
                    ...
                    send_ws_msg(
                        ...
                        data=dict(
                            device=device,
                            result="OK",
                            content="自定义服务对象识别……OK!",
                        ),
                    )
            log.debug("华三 data_dict: %s", data_dict)
            send_ws_msg(
                group_name=room_group_name,
                data=dict(
                    device=device, result="OK", content="开始编辑安全策略规则……!"
                ),
            )
            if is_top:
                config_policy_res, policy_content = _H3cFirewall.config_sec_policy(
                    config_data=data_dict, rule_id=rule_id, top=True
                )
            else:
                config_policy_res, policy_content = _H3cFirewall.config_sec_policy(
                    config_data=data_dict, rule_id=rule_id, top=False
                )
            log.debug(
                "华三 config_sec_policy 结果: res=%s, content=%s",
                config_policy_res,
                policy_content,
            )
            ...
            if config_policy_res:
                ...
                send_ws_msg(
                    ...
                    data=dict(
                        device=device,
                        result="Failed",
                        content="新建安全策略失败:" + policy_content,
                    ),
                )
...
def address_set(self, **post_param):
    ...
    if post_param["vendor"] == "H3C":
        ...
        # 地址组条目操作
        if (
            "add_detail_ip" in post_param.keys()
            or "add_detail_range" in post_param.keys()
        ):
            # step 1 生成netconf xml 配置
            post_param["method"] = "create"
        # 地址组条目操作
        elif (
            "del_detail_ip" in post_param.keys()
            or "del_detail_range" in post_param.keys()
        ):
            # step 1 生成netconf xml 配置
        ...
        _data = dict(
            order_code=(
                post_param.get("order_code") if post_param.get("order_code") else " "
            ),
...
def service_set(self, **post_param):
    ...
    if post_param["vendor"] == "H3C":
        ...
        log.debug("config_dnat (H3C) flow_data: %s", _data)
        _FirewallMain.flow_engine(*[cmds, back_off_cmds, class_method], **_data)
    return
...
def config_snat(self, **post_param):
    ...
    if post_param["vendor"] == "H3C":
        ...
        log.debug("config_snat (H3C) cmds: %s", cmds)
        # step 2 生成流程
        _data = dict(
        ...
        try:
            ...
            log.debug(
                "config_snat (Hillstone) cmds=%s, back_off=%s", cmds, back_off_cmds
            )
            _data = dict(
                order_code=(
                    post_param.get("order_code")
...
def config_dnat(self, **post_param):
    ...
    if post_param["vendor"] == "H3C":
        ...
        try:
            ...
            log.error("config_dnat (Hillstone) 未知异常:\n%s", traceback.format_exc())
            send_msg_sec_manage(
                "安全纳管引擎\n公网DNAT发布\n任务状态：失败\n原因:生成配置过程异常\n提示:{}".format(
                    str(e)
                )
...
def config_slb(self, **post_param):
    ...
    if post_param.get("vendor") == "H3C":
        ...
        log.debug("config_snat (Hillstone) flow_data: %s", _data)
        _FirewallMain.flow_engine(*[cmds, back_off_cmds, class_method], **_data)
        # tmp = {
        #     "task_id": str(self.request.id),
...
def config_sec_policy(self, **post_param):
    ...
    if post_param.get("vendor") == "H3C":
        ...
        class_method = "Hillstone"  # 类方法，山石直接下发命令，所以不需要，主要给华三华为对应对应类方法使用
        try:
            cmds, back_off_cmds = _FirewallMain.hillstone_sec_policy_detail(
                **post_param
            )
            log.debug("config_sec_policy (Hillstone) cmds:\n%s", "\n".join(cmds))
            log.debug(
                "config_sec_policy (Hillstone) back_off:\n%s", "\n".join(back_off_cmds)
            )
            _data = dict(
                order_code=(
                    post_param.get("order_code")
            ...
            log.debug("config_sec_policy (Hillstone) flow_data: %s", _data)
            _FirewallMain.flow_engine(*[cmds, back_off_cmds, class_method], **_data)
        except Exception as e:
            log.error(
                "config_sec_policy (Hillstone) 生成配置异常:\n%s",
...
def bulk_deny_by_address(self, **post_param):
    ...
    hosts = (
        AutomationInventory.objects.prefetch_related("ans_group_hosts")
        .filter(id=int(post_param["inventory_id"]))
        .values(
            "ans_group_name",
    ...
    def _build_post_data(host_vars, extra_fields):
        """构造 address_set 任务参数，合并公共字段与差异字段。"""
        base = {
    ...
    for host in hosts:
        ...
        try:
            ...
            log.debug("host_vars 解析成功: %s (type=%s)", host_vars, type(host_vars))
        except (json.JSONDecodeError, TypeError):
            log.warning(
                "ans_vars 字段非标准 JSON，尝试 ast.literal_eval 解析。host=%s",
                host.get("ans_group_hosts__ans_host"),
            )
            _parsed = ast.literal_eval(host["ans_group_hosts__ans_vars"])
            ...
            if post_param.get("add_detail_ip"):
                tasks.append(
                    address_set.s(
                        **_build_post_data(
                            host_vars,
                            {"add_detail_ip": True, "ip_mask": post_param["ip_mask"]},
                        )
                    )
                )
                ...
                tasks.append(
                    address_set.s(
                        **_build_post_data(
                            host_vars,
                            {"del_detail_ip": True, "ip_mask": post_param["ip_mask"]},
                        )
                    )
                )
    ...
    if tasks:
        group(tasks).apply_async(queue=CELERY_QUEUE, retry=True)
        log.info("bulk_deny_by_address 并行分发 %d 个 address_set 任务", len(tasks))
    return
...
def config_auto_switch(self, **post_param):
    ...
    if not post_param["batch"]:
        if switch_failback["status"] == "cover":
            SwitchFailBackDB.update(
                filter={"id": switch_failback["id"]},
                update={"$set": {"status": "recover", "last_result": flow_record.code}},
            )

        else:
            SwitchFailBackDB.update(
                filter={"id": switch_failback["id"]},
                update={"$set": {"status": "cover", "last_result": flow_record.code}},
            )
    else:
