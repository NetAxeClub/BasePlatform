...
COLLECTION_BINDING_ANALYSIS = MongoOps(db='Automation', coll='DevicePlanBindingAnalysisChecklist')

# 入库的详细数据
device_identity_mongo = MongoOps(db='Automation', coll='plan_device_identity')
arp_mongo = MongoOps(db='Automation', coll='ARPTable')          # arp类型
mac_mongo = MongoOps(db='Automation', coll='MACTable')
mac_evpn_mongo = MongoOps(db='Automation', coll='plan_mac_evpn')
...
aggre_port_mongo = MongoOps(db='Automation', coll='AggreTable')
...
snat_mongo = MongoOps(db='Automation', coll='SNAT')
